...
/// Export numbers as JSON numbers and model the workbench serializer's timestamp additions.
pub fn client_types() -> String {
    ...
    output.push_str("type StampedTurnEvent<E> = E extends { method: infer M; params: infer P } ? { sessionRevision: number; method: M; params: M extends 'turn/started' | 'tool/execution/start' ? Omit<P, 'startedAt'> & { startedAt: string } : P } : never;\n\n");
    output.push_str(
        &bindings
            .declarations
            .into_values()
