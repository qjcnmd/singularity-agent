...
14. Preserved Codex's dedicated Sandbox Users read principal, asynchronous read-grant helper, and best-effort `NUL` compatibility grant. Deny-read remains synchronous on that same principal, while its persistent state is strengthened with a global cross-process mutex, atomic replacement, and a monotonic path union so concurrent workspaces cannot revoke one another's protection. The `WRITE_RESTRICTED` fallback explicitly rejects deny-read overrides.
