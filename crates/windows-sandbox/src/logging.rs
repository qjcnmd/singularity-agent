...
use crate::product_identity::LOG_FILE_PREFIX;
use crate::string_utils::take_bytes_at_char_boundary;
...
mod tests {
    ...
    fn log_note_writes_to_daily_rolling_log() {
        ...
        assert!(filename.starts_with(&format!("{LOG_FILE_PREFIX}.")));
    ...
    fn log_file_path_for_utc_date_matches_rolling_appender_name() {
        ...
        assert_eq!(
            ...
            PathBuf::from("logs").join(format!("{LOG_FILE_PREFIX}.2026-05-21.log"))
        );
    }

    #[test]
    fn current_log_file_path_for_sandbox_home_uses_sandbox_dir() {
        let sandbox_home = Path::new("singularity-home");

        assert_eq!(
            current_log_file_path_for_sandbox_home(sandbox_home),
            current_log_file_path(
                &sandbox_home.join(crate::product_identity::SANDBOX_STATE_DIR_NAME)
            )
        );
    }
}
