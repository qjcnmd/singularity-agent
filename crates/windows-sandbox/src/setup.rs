...
use crate::product_identity::{
    ...
    SANDBOX_BIN_DIR_NAME, SANDBOX_SECRETS_DIR_NAME, SANDBOX_STATE_DIR_NAME,
    SANDBOX_USERS_FILE_NAME, SETUP_BINARY_NAME, SETUP_MARKER_FILE_NAME,
};
...
pub const OFFLINE_USERNAME: &str = OFFLINE_ACCOUNT_NAME;
pub const ONLINE_USERNAME: &str = ONLINE_ACCOUNT_NAME;
const ERROR_CANCELLED: u32 = 1223;
const SECURITY_BUILTIN_DOMAIN_RID: u32 = 0x0000_0020;
const DOMAIN_ALIAS_RID_ADMINS: u32 = 0x0000_0220;
const SETUP_EXE_FILENAME: &str = SETUP_BINARY_NAME;
...
pub fn sandbox_dir(sandbox_home: &Path) -> PathBuf {
    sandbox_home.join(SANDBOX_STATE_DIR_NAME)
}

pub fn sandbox_bin_dir(sandbox_home: &Path) -> PathBuf {
    sandbox_home.join(SANDBOX_BIN_DIR_NAME)
}

pub fn sandbox_secrets_dir(sandbox_home: &Path) -> PathBuf {
    sandbox_home.join(SANDBOX_SECRETS_DIR_NAME)
}

pub fn setup_marker_path(sandbox_home: &Path) -> PathBuf {
    sandbox_dir(sandbox_home).join(SETUP_MARKER_FILE_NAME)
}

pub fn sandbox_users_path(sandbox_home: &Path) -> PathBuf {
    sandbox_secrets_dir(sandbox_home).join(SANDBOX_USERS_FILE_NAME)
}
...
fn run_setup_refresh_inner(
    ...
    let cleared_report = match clear_setup_error_report(request.sandbox_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
    ...
    let cwd = std::env::current_dir().unwrap_or_else(|_| request.sandbox_home.to_path_buf());
    log_note(
        &format!(
    ...
    if let Err(err) = clear_setup_error_report(request.sandbox_home) {
        log_note(
...
const ALLOW_LOCAL_BINDING_ENV_KEY: &str = NETWORK_ALLOW_LOCAL_BINDING_ENV;
...
fn find_setup_exe() -> Result<PathBuf> {
    ...
    Err(anyhow!(
        "setup helper was not found next to the current executable or under {RESOURCES_DIRNAME}"
    ))
}
...
pub fn run_elevated_provisioning_setup(sandbox_home: &Path, real_user: &str) -> Result<()> {
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, sandbox_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to the sandbox home or anything under the sandbox home state directory,
    // the sandbox home helper directory, or the sandbox home secrets directory. These locations contain sandbox
...
mod tests {
    ...
    fn setup_exe_lookup_checks_package_resource_dir_for_bin_exe() {
        ...
        let exe = bin_dir.join("sg.exe");
        let setup_exe = resources_dir.join(crate::product_identity::SETUP_BINARY_NAME);
    ...
    fn offline_proxy_settings_ignore_proxy_env_when_online_identity_selected() {
        ...
        env.insert(
            crate::product_identity::NETWORK_ALLOW_LOCAL_BINDING_ENV.to_string(),
            "1".to_string(),
        );
    ...
    fn expanded_write_roots_still_drop_protected_sandbox_home() {
        let tmp = TempDir::new().expect("tempdir");
        let user_profile = tmp.path().join("user-profile");
        let sandbox_home = user_profile.join("SingularityHome");
        let documents = user_profile.join("Documents");
    ...
    fn effective_write_roots_match_payload_filtering_for_overrides() {
        ...
        let forbidden_sandbox_home =
            dunce::canonicalize(&sandbox_home).expect("canonical singularity home");
    ...
    fn payload_deny_write_paths_merge_explicit_and_protected_children() {
        ...
        let extra_singularity = extra_write_root.join(".singularity");
        let explicit_deny = tmp.path().join("explicit-deny");
        fs::create_dir_all(&command_git).expect("create command .git");
        fs::create_dir_all(&extra_singularity).expect("create extra .singularity");
        let writable_roots = vec![
