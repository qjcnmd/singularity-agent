...
mod windows_impl {
    ...
    pub fn run_windows_sandbox_capture_with_filesystem_overrides(
        ...
        if !requested_permissions.supports_restricted_token_fallback() {
            anyhow::bail!(
                "unsupported: restricted-token fallback cannot enforce restricted network access; use the elevated offline identity"
            );
        }
