...
use singularity_windows_sandbox::dpapi_protect;
use singularity_windows_sandbox::product_identity::{
    SANDBOX_USERS_FILE_NAME, SANDBOX_USERS_GROUP_COMMENT, SANDBOX_USERS_GROUP_NAME,
...
pub const SANDBOX_USERS_GROUP: &str = SANDBOX_USERS_GROUP_NAME;
...
fn write_secrets(
    ...
    let secrets_dir = sandbox_secrets_dir(sandbox_home);
    ...
    let users_path = secrets_dir.join(SANDBOX_USERS_FILE_NAME);
...
pub(super) fn prepare_setup_marker(sandbox_home: &Path, real_user: &str) -> Result<()> {
    let marker_path = sandbox_dir(sandbox_home).join(SETUP_MARKER_FILE_NAME);
