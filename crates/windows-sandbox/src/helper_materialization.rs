...
use crate::product_identity::{
    COMMAND_RUNNER_BINARY_NAME, HELPER_BIN_DIR_NAME, HELPER_RESOURCES_DIR_NAME,
};
...
impl HelperExecutable {
    fn file_name(self) -> &'static str {
        match self {
            Self::CommandRunner => COMMAND_RUNNER_BINARY_NAME,
        }
    }
...
mod tests {
    ...
    use crate::product_identity::COMMAND_RUNNER_BINARY_NAME;
    ...
    fn helper_bin_dir_is_under_sandbox_bin() {
        let sandbox_home = Path::new(r"C:\Users\example\.singularity");

        assert_eq!(
            PathBuf::from(r"C:\Users\example\.singularity\.sandbox-bin"),
    ...
    fn copy_runner_into_shared_bin_dir() {
        ...
        let runner_source = source_dir.join(COMMAND_RUNNER_BINARY_NAME);
    ...
    fn helper_source_lookup_checks_resource_dir() {
        ...
        let exe = release_dir.join("sg.exe");
        let helper = resources_dir.join(COMMAND_RUNNER_BINARY_NAME);
    ...
    fn helper_source_lookup_prefers_package_resource_dir_over_bin_resource_dir() {
        ...
        let exe = bin_dir.join("sg.exe");
        let package_helper = package_resources_dir.join(COMMAND_RUNNER_BINARY_NAME);
        let bin_helper = bin_resources_dir.join(COMMAND_RUNNER_BINARY_NAME);
        fs::write(&exe, b"singularity").expect("write exe");
    ...
    fn helper_source_lookup_prefers_direct_sibling_over_resource_dir() {
        ...
        fs::create_dir_all(&resources_dir).expect("create resources dir");
        let exe = release_dir.join("sg.exe");
        let sibling_helper = release_dir.join(COMMAND_RUNNER_BINARY_NAME);
        let resource_helper = resources_dir.join(COMMAND_RUNNER_BINARY_NAME);
        fs::write(&exe, b"singularity").expect("write exe");
    ...
    fn materialized_file_name_adds_suffix_before_extension() {
        ...
        assert_eq!(
            ...
            format!(
                "{}-test-suffix.exe",
                COMMAND_RUNNER_BINARY_NAME.trim_end_matches(".exe")
            )
        );
    }
}
