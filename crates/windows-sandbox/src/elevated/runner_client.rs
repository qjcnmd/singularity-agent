...
fn connect_pipe_with_timeout(
    ...
    let mut connect_thread = Some(
        thread::Builder::new()
            .name(format!("{RUNNER_CONNECT_THREAD_PREFIX}-{pipe_label}"))
            .spawn(move || {
...
pub(crate) fn spawn_runner_transport(
    ...
    if let Err(err) = connect_result {
        unsafe {
            ...
            // that child instead of leaking a stray the command runner.
            if pi.hProcess != 0 {
    ...
    let startup_result = (|| -> Result<()> {
        ...
        // runner instead of leaving a stray the command runner behind.
