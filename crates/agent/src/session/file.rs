...
/// 会话工作目录的唯一呈现形状：正斜杠绝对路径，且不带 Windows verbatim 前缀。
///
/// `\\?\` 与 `\\?\UNC\` 前缀是为绕过 Win32 路径解析而存在的内部表示：它在 shell
/// 命令里不可用，也不向用户或模型传达任何额外信息；Rust 标准库在需要（例如超长
/// 路径）时自行补该前缀，所以呈现时剥掉不损失可访问性。历史会话头里可能仍带前缀，
/// 经这里归一后即收敛到同一形状。
...
/// 会话头里 `cwd` 字段的唯一形状：一律归一为绝对路径、正斜杠、无 Windows
/// verbatim 前缀。存量文件可能记着 `//?/C:/…`，`std::path::absolute` 先把它
/// 还原成原生形状，再交 [`normalize_cwd_string`] 剥前缀并统一分隔符，因此读取
/// 侧不需要知道历史写法。
pub(crate) fn normalize_cwd_text(stored: &str) -> Result<String> {
    let path = std::path::absolute(stored).map_err(|error| {
        SessionError::InvalidHeader(format!("header cwd is not a usable path: {error}"))
    })?;
    Ok(normalize_cwd_string(&path))
}
