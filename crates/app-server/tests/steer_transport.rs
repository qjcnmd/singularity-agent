...
fn active_turn_settings_queue_until_terminal_then_index_syncs() {
    ...
    let home = home_dir.path().to_path_buf();
    let provider = ScriptedProvider::start(interrupt_responses());
    // 双模型用户配置目录：环境层会覆盖用户配置层，因此这里不放任何
    // SINGULARITY_MODEL/BASE_URL/API_KEY，快照完全来自 config.json。
    std::fs::write(
        ...
        serde_json::to_string_pretty(&json!({
            ...
            "providers": {
                "openai_compatible": {
                    ...
                    "models": {
                        "test-model": { "api_protocol": "responses" },
                        "test-model-b": { "api_protocol": "responses" }
                    }
                }
            }
        }))
    ...
    // 空闲设置：立即生效并同步索引，thread/list 反射新 selector。
    process.send_request(
        4,
        "thread/settings",
        json!({"threadId": session_id, "model": "test-model-b"}),
    );
    let idle_settings = process.output.recv_id(4, Duration::from_secs(5));
    assert_eq!(idle_settings["result"]["updated"], true);
    assert_eq!(idle_settings["result"]["queued"], false);
    assert_eq!(
        listed_model(&mut process, 5).as_deref(),
        Some("openai_compatible/test-model-b")
    );

    // 活动轮设置：queued=true；索引保持旧 model。
    process.send_request(
        6,
        "turn/start",
        json!({"threadId": session_id, "input": [{"type": "text", "text": "run long tool"}]}),
    ...
    process.send_request(
        7,
        "thread/settings",
        json!({"threadId": session_id, "model": "test-model"}),
    );
    let active_settings = process.output.recv_id(7, Duration::from_secs(5));
    assert_eq!(active_settings["result"]["updated"], true);
    assert_eq!(active_settings["result"]["queued"], true);
    assert_eq!(
        listed_model(&mut process, 8).as_deref(),
        Some("openai_compatible/test-model-b"),
        "index must keep the persisted model while the intent is queued"
    );

    // 中断终态：runtime 持久化并发布 settingsApplied（位于 turn 终态事件
    // 之后）；turn/start 响应只在链条完全收敛后返回，此时索引已同步。
    process.send_request(9, "turn/interrupt", json!({"turnId": turn_id}));
    let interrupt = process.output.recv_id(9, Duration::from_secs(5));
    assert_eq!(interrupt["result"]["status"], "interrupted");
    let turn_response = process.output.recv_id(6, Duration::from_secs(30));
    assert_eq!(turn_response["result"]["turn"]["status"], "running");
    let terminal = process
        .output
        .recv_where(Duration::from_secs(30), |message| {
    ...
    assert_eq!(terminal["params"]["turn"]["status"], "interrupted");
    // 终态后 runtime 持久化并发布 settingsApplied：索引随事件同步，客户端
    // 轮询 thread/list 在界内收敛到应用后的 selector。
    let deadline = std::time::Instant::now() + Duration::from_secs(5);
    let mut final_model = None;
    while std::time::Instant::now() < deadline {
        final_model = listed_model(&mut process, 10);
        if final_model.as_deref() == Some("openai_compatible/test-model") {
            break;
        }
    ...
    assert_eq!(
        ...
        "settingsApplied must have synced the index with the applied selector; {}",
        process.output.failure_context()
    );
...
/// thread/list 中首个 thread 的 model 选择器。
fn listed_model(process: &mut AppServerProcess, id: i64) -> Option<String> {
    process.send_request(id, "thread/list", json!({}));
    let listed = process.output.recv_id(id, Duration::from_secs(5));
    listed["result"]["threads"][0]["model"]
        .as_str()
        .map(str::to_string)
}
