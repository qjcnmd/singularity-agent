...
test('connection keeps retrying after a long outage and stops cleanly', t => {
  const { connection, timers, sockets, statuses, frames, runTimer } = connectionFixture(t)
  connection.start()
...
test('an unreadable or mismatched RPC response reconciles state once without replaying the mutation', async t => {
  const { connection, timers, statuses, runTimer } = connectionFixture(t)
  let calls = 0
  connection.start()
  ...
  for (const envelope of [
    ...
    assert.equal(timers.size, 1, 'exactly one state reconciliation is scheduled')
    runTimer()
  }
...
function connectionFixture(t) {
  ...
  return { connection, timers, sockets, statuses, frames, runTimer: () => {
