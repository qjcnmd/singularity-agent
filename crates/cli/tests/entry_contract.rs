...
/// 在隔离 home 写入指向本地假 Chat 服务器的用户配置（config.json + auth.json）。
fn install_fake_provider(home: &Path, base_url: &str) {
    let config = serde_json::json!({
        "version": 1,
        "default_provider": "openai_compatible",
        "default_model": "openai_compatible/fake-model",
        "providers": {
            "openai_compatible": {
                "base_url": base_url,
                "models": {
...
fn print_writes_only_final_assistant_text() {
    ...
    install_fake_provider(&home.path, &base_url);
    let output = sg()
...
fn length_truncation_is_persisted_and_projected_by_headless_modes() {
    ...
    install_fake_provider(&print_home.path, &base_url);
    let output = sg()
    ...
    install_fake_provider(&json_home.path, &base_url);
    let output = sg()
