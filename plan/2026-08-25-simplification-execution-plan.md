...
- 收尾-提交A — commit `6bc5720`; 四项结构收尾（REQ-023 漏项装配 helper、六工具参数解析错误处理共享、transport 过期转发层内联、jsonl emit_summary 单一入口），行为不变。五项门禁全绿：fmt --check / clippy -D warnings / test --workspace（agent 71、runtime 15、cli 21+12+4、app-server 35+4+3、model 38+24+23+22、core 3、protocol 12、provider smoke 1 ignored）/ build --bins / git diff --check，工作树 clean。偏差：独立双轴审查子代理运行失败两次，改由主代理完成审查确认。
- 收尾-提交B — commit `16d5087`; docs/singularity.md 补 `--json` 终态 summary 可选 `truncated` 字段与 reasoning 三态语义（wire 缺字段=Keep、null=Clear、字符串=Set；TUI 清空=Clear；app-server 原样映射、null 计更新）当前事实。五项门禁全绿，工作树 clean。偏差：无。
