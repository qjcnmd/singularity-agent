...
class TurnCoordinator:
    def __init__(
        ...
        *,
        ...
        self.record_model_failure = record_model_failure
        self.outcome_from_model_failure = outcome_from_model_failure
        self.terminal_result_from_outcome = terminal_result_from_outcome
        self.record_outcome_context = record_outcome_context
        self.assistant_message_from_result = assistant_message_from_result
        self.attempt_finalize = attempt_finalize
        self.maybe_analyze_failure = maybe_analyze_failure
        self.should_auto_finalize_after_tools = should_auto_finalize_after_tools
    ...
    def run_turn(
        ...
        *,
        ...
        if result.status != ModelTurnStatus.SUCCESS:
            self.record_model_failure(planner, result, turn=turn)
            outcome = self.outcome_from_model_failure(result)
            controller.apply_outcome(outcome)
            self.record_outcome_context(context, planner, outcome)
            terminal = self.terminal_result_from_outcome(outcome, turn=turn)
            if terminal is not None:
                return terminal
            return None

        assistant_message = self.assistant_message_from_result(result)
        if not result.tool_calls:
            context.add_assistant_message(assistant_message)
            final = self.attempt_finalize(
                planner,
        ...
        if reduced_outcome is not None:
            ...
            self.record_outcome_context(context, planner, reduced_outcome)
            ...
            if blocked is not None:
                ...
                terminal = self.terminal_result_from_outcome(blocked, turn=turn)
                if terminal is not None:
                    return terminal
            terminal = self.terminal_result_from_outcome(reduced_outcome, turn=turn)
            if terminal is not None:
                return terminal
        blocked = self.maybe_analyze_failure(
            planner,
        ...
        if blocked is not None:
            ...
            if terminal is not None:
                return terminal
        if self.should_auto_finalize_after_tools(planner, protocol_result):
            final = self.attempt_finalize(
                planner,
                controller=controller,
                context=context,
