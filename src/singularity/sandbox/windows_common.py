...
@lru_cache(maxsize=1)
def _probe_windows_sandbox_cached_impl() -> WindowsSandboxDoctorReport:
...
def _apply_sandbox_control_dir_acl(
    ...
    *,
    ...
    from singularity.sandbox.windows_acl import _apply_sandbox_control_dir_acl as owner
...
def _ensure_sandbox_identity(identity: _WindowsSandboxIdentity) -> _OperationResult:
    from singularity.sandbox.windows_identity import _ensure_sandbox_identity as owner

    return owner(identity)


def _network_state(sid: str) -> WindowsCapabilityState:
    from singularity.sandbox.windows_firewall import _network_state as owner

    return owner(sid)


def _runner_smoke_state(identity: _WindowsSandboxIdentity) -> WindowsCapabilityState:
    from singularity.sandbox.windows_runtime import _runner_smoke_state as owner

    return owner(identity)


def probe_windows_sandbox() -> WindowsSandboxDoctorReport:
    from singularity.sandbox.windows_doctor import probe_windows_sandbox as owner

    return owner()


def _clear_probe_windows_sandbox_cache() -> None:
    from singularity.sandbox.windows_doctor import probe_windows_sandbox as owner

    owner.cache_clear()


probe_windows_sandbox.cache_clear = _clear_probe_windows_sandbox_cache  # type: ignore[attr-defined]


def cleanup_windows_sandbox_assets() -> WindowsSandboxCleanupReport:
    from singularity.sandbox.windows_cleanup import cleanup_windows_sandbox_assets as owner
