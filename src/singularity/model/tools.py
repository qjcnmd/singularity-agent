...
class ModelToolRenderer:
    ...
    def render(self, *, allowed_tool_names: list[str] | None = None, strict: bool = False) -> list[ModelToolSchema]:
        ...
        exported = {tool["name"]: tool for tool in self.registry.schema_export(strict=strict)}
        ...
        for spec in sorted(visible, key=lambda item: item.name):
            if spec.name not in allowed:
                continue
            neutral_schema = exported.get(spec.name)
            if not neutral_schema:
                continue
            ...
            schemas.append(
                ModelToolSchema(
                    ...
                    parameters_schema=dict(neutral_schema.get("input_schema") or {}),
