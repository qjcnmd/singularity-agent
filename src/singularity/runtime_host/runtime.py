...
class RuntimeHost:
    ...
    def start_run(
        ...
        *,
        ...
        resolved_config = config or ProductionRuntimeConfig.from_cli(project_root=self.project_root)
    ...
    def resume_run(
        ...
        *,
        ...
        resolved_config = config or ProductionRuntimeConfig.from_cli(
            project_root=self.project_root,
            resume_session=session_id,
        )
        return self.start_run(goal, config=resolved_config)

    def cancel_run(self, run_id: str, *, message: str = "cancelled by RuntimeHost") -> RuntimeHostSnapshot:
        kernel = self._kernels.get(run_id)
        if kernel is None:
            raise RuntimeHostError(f"Unknown active run: {run_id}")
    ...
    def submit_approval(self, run_id: str, grant: ApprovalGrant | dict[str, Any]) -> ApprovalEvent:
        ...
        policy_runtime = getattr(kernel.graph, "policy_runtime", None)
        if policy_runtime is None or not hasattr(policy_runtime, "register_grant"):
            raise RuntimeHostError("Active run does not expose PolicyRuntime.register_grant.")
        policy_runtime.register_grant(approval_grant)
    ...
    def snapshot(self, run_id: str) -> RuntimeHostSnapshot:
