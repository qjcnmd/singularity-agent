...
_CompactionGroup: TypeAlias = CompactionGroup
_CompactionPlan: TypeAlias = CompactionPlan
...
class ContextManager:
    def __init__(
        ...
        *,
        ...
        self.usage_reporter = ContextUsageReporter(self)
    ...
    def record_model_usage(self, result: Any) -> None:
        self.usage_reporter.record_model_usage(result)

    def context_usage_diagnostic(self) -> dict[str, Any]:
        return self.usage_reporter.diagnostic()
    ...
    def _compress_if_possible(
        ...
        *,
        ...
        if self.provider is None and self.model_runner is None:
            return False
        plan = self._prepare_compaction_plan(
    ...
    def _prepare_compaction_plan(
        ...
        *,
        ...
        return self.compaction_planner.prepare(
            focused_item_ids=focused_item_ids,
            partial_range=partial_range,
        )
    ...
    def _recent_tail_messages(
        ...
        *,
        ...
        bounded = [
            ...
            and (partial_range is None or not _message_in_partial_range(message, partial_range))
        ]
        return bounded[-COMPACTION_RECENT_TAIL_MESSAGES:]

    @staticmethod
    def _item_in_partial_range(
        item: ContextItem,
        compaction_range: PartialCompactionRange,
    ) -> bool:
        turn = _metadata_turn(item.metadata)
        if turn is not None:
        ...
        *,
        ...
        if partial_range is not None:
            preserved = [
                ...
                if not _message_in_partial_range(message, partial_range)
            ]
        return [
            *base,
...
def _metadata_turn(metadata: dict[str, Any]) -> int | None:
...
def _message_in_partial_range(
    ...
    turn = _metadata_turn(metadata)
    if turn is None and "turn" in message:
        turn = _metadata_turn(message)
    if turn is not None:
        if compaction_range.start_turn is not None and turn < compaction_range.start_turn:
            return False
        if compaction_range.end_turn is not None and turn > compaction_range.end_turn:
