...
from singularity.policy.permissions import PermissionProfile
    ...
    def __post_init__(self) -> None:
        ...
        if self.permission_profile is None:
            object.__setattr__(
                self,
                "permission_profile",
                PermissionProfile.default_for_workspace(root),
            )
        elif root not in self.permission_profile.workspace_roots:
            raise ValueError(
                "PolicyConfig workspace_root must be one of the permission profile workspace roots."
            )
