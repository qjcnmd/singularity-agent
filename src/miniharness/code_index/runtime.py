...
from miniharness.code_index.context import build_project_index_observation
...
from miniharness.code_index.language import LanguagePlugin
...
from miniharness.code_index.plugins import (
    JavaScriptPlugin,
    PythonPlugin,
    RustPlugin,
    TypeScriptPlugin,
...
class ProjectIndexRuntimeConfig:
    enabled: bool = True
...
class ProjectIndexRuntime:
    def __init__(
        ...
        *,
        ...
        trace: Any | None = None,
        config: ProjectIndexRuntimeConfig | None = None,
        ...
        self.db_path = Path(db_path) if db_path else self.config.db_path or self.workspace_root / ".miniharness" / "index.sqlite"
        self.trace = trace
        ...
        for candidate in sorted(set(candidates) & existing):
            mappings.append(
                TestMappingRecord(
                    ...
                    confidence=0.55,
                    evidence=[Evidence(source="project_index_runtime", path=candidate)],
                    trust_level=TrustLevel.RUNTIME_GENERATED,
                    backend=BackendInfo(name="project_index_runtime", version=SCHEMA_VERSION),
                    source="project_index_runtime",
                )
            )
    return mappings
