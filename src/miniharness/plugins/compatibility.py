...
def check_compatibility(plugin: DiscoveredPlugin | PluginManifest) -> list[PluginDiagnostic]:
    ...
    if compatibility.min_miniharness_version and mh_version < _version_tuple(
        compatibility.min_miniharness_version
    ):
        diagnostics.append(
            _diagnostic(
                manifest.id,
                "miniharness_version_too_old",
                "Current MiniHarness version is below plugin minimum.",
                path=path,
            )
        )
    if compatibility.max_miniharness_version and mh_version > _version_tuple(
        compatibility.max_miniharness_version
    ):
        diagnostics.append(
            _diagnostic(
                manifest.id,
                "miniharness_version_too_new",
                "Current MiniHarness version is above plugin maximum.",
                path=path,
            )
        )
