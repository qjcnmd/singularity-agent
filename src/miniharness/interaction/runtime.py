...
from miniharness.observability.models import TraceEvent, TraceEventType, TraceSeverity
...
class InteractionRuntime:
    ...
    def publish(
        self,
        event: RuntimeEvent | ProgressEvent,
        *,
        write_trace: bool = True,
    ) -> RuntimeEvent:
        runtime_event = event.to_runtime_event() if isinstance(event, ProgressEvent) else event
        self.events.append(runtime_event)
        if write_trace:
            self._write_trace(runtime_event)
        for sink in list(self.sinks):
            try:
                self._deliver(sink, runtime_event)
    ...
    def consume_trace_event(self, trace_event: TraceEvent) -> RuntimeEvent:
        runtime_event = runtime_event_from_trace_event(trace_event)
        return self.publish(runtime_event, write_trace=False)
        ...
        *,
        ...
        if self.mode == InteractionMode.NON_INTERACTIVE:
            ...
            answer = ClarificationAnswer(
                request_id=request.request_id,
                answer="",
                answered_by="interaction-runtime",
                metadata={"provider_missing": True, "fail_closed": True},
            )
        else:
    ...
    def _write_trace(self, event: RuntimeEvent) -> None:
    ...
    @staticmethod
    def _deliver(sink: Any, event: RuntimeEvent) -> None:
        if callable(sink):
            sink(event)
            return
...
def runtime_event_from_trace_event(event: TraceEvent) -> RuntimeEvent:
