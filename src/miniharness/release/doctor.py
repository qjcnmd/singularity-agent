...
from miniharness.release.init import load_config, validate_config
        ...
        if not path.exists():
            checks.append(
                ReleaseCheck(
                    ...
                    suggestion="Run miniharness system init or miniharness system repair.",
                )
            )
...
def _config_check(paths: RuntimePaths) -> ReleaseCheck:
    ...
    try:
        ...
        return ReleaseCheck(
            ...
            suggestion="Run miniharness system repair or restore from backup.",
        )
    return ReleaseCheck(
        "config_schema",
...
def _critical_runtime_config_check(paths: RuntimePaths) -> ReleaseCheck:
    ...
    missing_env = [
        name
        for name in ("MINIHARNESS_API_KEY", "MINIHARNESS_BASE_URL", "MINIHARNESS_MODEL")
    ...
    if missing_sections:
        return ReleaseCheck(
            ...
            suggestion="Run miniharness system repair.",
        )
    if missing_env:
        return ReleaseCheck(
            "runtime_configuration",
...
def _migration_check(paths: RuntimePaths) -> ReleaseCheck:
    if not paths.manifest_file.exists():
        return ReleaseCheck(
            ...
            suggestion="Run miniharness system init.",
        )
    try:
    ...
    if not schema_ok:
        return ReleaseCheck(
            ...
            suggestion="Run miniharness system migrate after backing up user data.",
    ...
    if pending:
        return ReleaseCheck(
            ...
            suggestion="Run miniharness system migrate.",
        )
