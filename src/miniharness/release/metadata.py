...
from miniharness import __version__
from miniharness.release.paths import RuntimePaths, resolve_runtime_paths
...
def package_version() -> str:
    ...
    try:
        metadata_version = importlib.metadata.version("miniharness")
    except importlib.metadata.PackageNotFoundError:
        return source_version or __version__
    if source_version and _source_root().joinpath("src", "miniharness").exists():
...
def requires_python() -> str:
    try:
        value = importlib.metadata.metadata("miniharness").get("Requires-Python")
    except importlib.metadata.PackageNotFoundError:
        value = None
    if value:
        return value
...
def _installed_package() -> bool:
    try:
        importlib.metadata.distribution("miniharness")
    except importlib.metadata.PackageNotFoundError:
        return False
    return True
...
def _install_path() -> Path:
    spec = importlib.util.find_spec("miniharness")
