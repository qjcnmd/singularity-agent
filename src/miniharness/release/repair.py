...
from miniharness.release.migrations import load_manifest
...
def export_user_data(paths: RuntimePaths, output: Path | str) -> dict[str, Any]:
    ...
    manifest = {
        "schema_version": "miniharness.export/v1",
...
def _runtime_is_owned(paths: RuntimePaths) -> tuple[bool, str | None]:
    if not paths.manifest_file.exists():
        return False, f"MiniHarness runtime manifest not found: {paths.manifest_file}"
    try:
        manifest = load_manifest(paths)
    except Exception as exc:
        return False, f"MiniHarness runtime manifest is unreadable: {type(exc).__name__}: {exc}"
    if not manifest.app_version:
        return False, "MiniHarness runtime manifest is missing app_version."
    return True, None
