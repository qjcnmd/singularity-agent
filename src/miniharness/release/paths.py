...
class RuntimePaths:
    ...
    def config_file(self) -> Path:
        return self.config_dir / "miniharness.json"
...
def resolve_runtime_paths(
    *,
    ...
    env_home = os.getenv("MINIHARNESS_HOME")
    if home is not None:
        root = Path(home).expanduser()
    elif env_home:
        root = Path(env_home).expanduser()
    elif resolved_mode == RuntimeMode.DEVELOPMENT:
        root = Path(project_root or Path.cwd()).expanduser() / ".miniharness"
...
def _mode(value: RuntimeMode | str | None) -> RuntimeMode:
    raw = value or os.getenv("MINIHARNESS_MODE") or RuntimeMode.USER.value
    try:
        return RuntimeMode(str(raw).lower())
    except ValueError as exc:
        raise ValueError("MINIHARNESS_MODE must be user, development, or portable.") from exc
