...
function TrajectoryView() {
  ...
  useLayoutEffect(() => {
    if (!selected && follow.current && container.current) container.current.scrollTop = container.current.scrollHeight
  }, [rows, selected])
      ...
      <table aria-label="轨迹记录"><tbody>{rows.map(row => {
        ...
        return <motion.tr layout="position" transition={transition} key={row.key} data-trajectory-id={row.key} className={item.status === 'error' ? 'is-failed' : undefined}>
          ...
          <td className="trajectory-preview-cell"><button type="button" className="trajectory-preview" onClick={() => { if (!hasTextSelection()) inspect(row) }}>
            {item.kind === 'tool' && <><strong className="execution-title">{item.title}</strong><JsonPreview value={item.input} />{item.text && <span className="trajectory-arrow">→</span>}</>}
...
function Inspector({ row, request, tab, setTab, onRequest }: { row: Row; request: boolean; tab: TabId; setTab: (tab: TabId) => void; onRequest: () => void }) {
  ...
  return <>
    ...
    <AnimatePresence initial={false} mode="wait"><motion.div key={active} className="trajectory-detail-body" role="tabpanel" aria-label={tabs.find(entry => entry.id === active)?.label} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: reducedMotion ? 0 : 0.12 }}>
      ...
      {active === 'summary' && <><dl className="trajectory-facts"><div><dt>状态</dt><dd>{factStatusText[item.status]}</dd></div>{stats && <><div><dt>提供方</dt><dd>{stats.provider}</dd></div><div><dt>模型</dt><dd>{stats.model}</dd></div></>}<div><dt>耗时</dt><dd>{seconds(item.duration)}</dd></div>{item.status === 'error' && stats?.error && <div><dt>错误</dt><dd>{stats.diagnosticCode ? `${stats.error}（${stats.diagnosticCode}）` : stats.error}</dd></div>}{item.error && <div><dt>原因</dt><dd>{item.error.cause}</dd></div>}</dl>
      ...
      {active === 'input' && <JsonValue value={item.input} />}
      {active === 'schema' && <JsonValue value={item.schema} />}
      {active === 'raw' && <JsonValue value={request ? stats : item} />}
      {active === 'output' && <Payload text={item.text || '未记录'} />}
      {active === 'thinking' && <Payload text={item.thinking || '未记录'} />}
...
const JsonPreview = memo(function JsonPreview({ value }: { value: unknown }) {
  const text = useMemo(() => pretty(value), [value])
  return <code>{text}</code>
})
const JsonValue = memo(function JsonValue({ value }: { value: unknown }) {
  const text = useMemo(() => pretty(value), [value])
  return <Payload text={text || '未记录'} />
})
