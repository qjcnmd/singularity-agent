...
启动任何模型 trial 前，runner 先在本次 run 所在真实文件系统的 run-owned scratch workspace 上调用选定 `SandboxBackend` 的完整 preflight，并用同一 trusted workspace-preparation 边界执行一次固定的断网 `git init`，证明 clone/checkout 所需的受保护 Git metadata 变化也能被严格 backend 确认。报告固定记录 profile、backend、OS/arch、kernel、filesystem，以及 OverlayFS、user/mount/PID/network namespace、no-new-privs、seccomp、Landlock、transactional workspace、network denied 和 protected-path enforcement。只有同一 backend 名称和全部严格合同均通过时才把该已验证 backend 传给所有 trial；trusted preparation 或其他能力缺失时只发布一个绑定稳定错误码的 environment blocker，保留 configured trial count，但 sampled trial count 为零，且不调用 Provider、不创建 trial。scratch 创建、清理或报告合同失败同样 fail closed，不在 publication 或 task workspace 上执行替代探针；没有 relaxed、local-process 或 no-sandbox fallback。

trusted workspace-preparation 的来源标志只由进程内 Rust API 构造，并传递到平台 adapter；它仍使用与普通命令相同的严格 sandbox、断网和 transactional workspace 合同，但把控制面创建或更新的 Git metadata 纳入完整事务快照、漂移检查与提交。可序列化请求、manifest 命令和模型工具调用不能获得该来源，仍按普通 protected-path 规则拒绝 `.git` 等路径。
