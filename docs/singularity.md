...
9. 没有 tool call 时应用 completion gate，接受或拒绝 final answer；repairable failure 或 completion/plan rejection 使状态仍不可 final 时，下一 `ModelTurnRequest` 保持 `Auto` 并携带 typed tool result 或固定 developer feedback。反馈同时列出全部未满足的 plan 与 verification 不变量，不以其中一个遮蔽另一个；普通文本不能绕过本地 completion gate。
