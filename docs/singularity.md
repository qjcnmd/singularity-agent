...
- `core` 是受保护元数据名称（`.git`、`.agents`、`.singularity`）的唯一事实源，`tools`、sandbox policy 和 Windows adapter 都从该策略投影；Windows writable root 缺失时只物化并保护 runtime-owned 的 `.agents`/`.singularity` sentinel，不物化缺失的 `.git`，以保留 Git ancestor discovery。
- Windows ACL 通过稳定句柄读取和写入，并在 reparse/最终对象身份不确定时返回 typed unsupported/error；真实 ACL 失败会贯穿 setup、restricted-token fallback 和 elevated caller 关闭失败。deny-read 状态按 workspace capability SID 归属，在命名跨进程互斥下以原子替换写入并只做单调保护集合合并，不使用共享 Sandbox Users SID 的 last-writer-wins/revoke。
