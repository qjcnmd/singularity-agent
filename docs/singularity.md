...
| AgentLoop execution | `AgentKernel -> AgentLoop.run` | Python sidecar only, invoked by `AppServer::run_python_sidecar_if_enabled` | `agent/run` or `agent/resume` over sidecar JSON-RPC | Rust trace summary is appended after terminal sidecar result; active run metadata is stored while running | Must enter existing Python `KernelBootstrap -> AgentKernel -> AgentLoop` for real model proof | Native Rust AgentLoop unavailable; sidecar lifecycle is Rust-owned but semantics stay Python-owned |
| Tool protocol execution | `ToolProtocolEngine` / `ToolExecutor` | Python sidecar owns execution during this stage | Sidecar result event summaries only | Rust stores safe `python_sidecar` trace summaries | Tool behavior remains Python oracle behavior | No Rust tool execution claim through CLI path |
| Provider/model call | Python `ModelRunner` | Python sidecar / Python AgentLoop | Thread `model` -> sidecar `model` parameter | Rust stores only safe status/trace refs | Provider config and calls remain existing Python behavior | Rust `crates/model` stays schema/validation-only |
...
| Evaluation proof | Python `EvaluationRunner` | Rust CLI sidecar smoke plus Python real eval proof | CLI `sg run --agent-host python`; Python `eval run` | Result/report/trace artifacts stay in Python work dirs | Must prove real AgentLoop, not fake/mock/scripted fallback | Full evaluation harness is not expressed natively by Rust CLI yet |

### Rust sidecar status and error map

| Source condition | AgentLoopStatusBridge | TurnStatus | CLI output | Trace summary |
| --- | --- | --- | --- | --- |
| Default Rust `turn/start` without sidecar | `not_migrated` | `running` | `turn <id> running agent_loop_status=not_migrated` | Only app-server turn trace; no `item/agentMessage/delta` |
