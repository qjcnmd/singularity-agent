...
- typed verification requirement 使用 canonical command scope 的 SHA-256 digest 和大于零的成功次数；要求的 digest/count 必须精确组成最后的成功 command 后缀多重集合。mutation 会清空后缀；其他成功 command 会占据后缀位置，直到其后重新运行完整 required commands 才能恢复终态。调用方经 `AgentLoopInput::with_verification_commands` 提交 exact command action 与成功次数，Runtime 先完整校验 action，再从 command、cwd、timeout 和 sandbox/network profile 派生 digest；模型不再被要求从不可逆摘要猜测调用方命令。派生出的 caller requirements 是完成门禁的硬下限：调用方内部重复 digest 按 `CompletionTracker` 的 checked-add 归约，模型 plan 由 `AgentVerificationPlan::requirements()` 按 digest 取 max，跨两者再按 digest 取 max；模型 plan 只能扩大、不能删除或缩减该下限。mutation 清空旧 evidence 与模型 plan 时保留下限，并把 caller-owned actions 作为有界可信 Developer 投影交给下一次 replanning；repair、pause/resume、ApprovalCheckpoint/TurnCheckpoint 恢复与 user steer 都从当前 input 的 commands 重新派生 requirements，按同一 union 语义重建并全等校验。
...
- Completion reducer 只在已经观察到真实 workspace mutation、且同一 revision 的全部 exact verification 已成功后，消解被输入边界拒绝且未执行的 edit/patch `invalid_tool_arguments` / `invalid_tool_input`；这类失败没有副作用，不能在可信 revision 已完整验证后永久污染完成状态。执行阶段失败、未知 workspace 变化以及 scope/revision 不完整仍保持 unresolved 并 fail closed。
- `WorkspaceTools` 为 edit/patch 和 backend 明确报告的 workspace-write command 变化分配单调的内部 workspace revision；edit/patch 的发布者同时从实际 prepared mutation 的相对路径、原正文 bytes 和发布后正文 bytes 计算 producer-owned `WorkspaceChangeSummary`。Windows/Linux sandbox 在启动 workspace-write child 前通过已打开的 workspace directory capability 建立有界、拒绝跟随链接的 metadata/content-digest 快照：普通文件、目录和链接进入可验证摘要。Linux 对 protected path 只保留不透明 metadata：受保护普通文件仍比较全部已采集 metadata，受保护目录及其普通祖先只比较存在、类型、权限与对象身份，目录内部由 trusted runtime 产生的 length、timestamp、link-count 和内容波动不属于 Agent 可见 workspace revision；对象替换、权限变化或普通 workspace 漂移仍 fail closed。Windows 在同一个 deny-read execution mutex 和 elevated transaction 内，以将实际启动 child 的受限账户身份建立 before/after snapshot；这样 child 新建的私有 ACL 对象仍可由同一安全主体观测，controller 不获得额外读取权限。已有 sandbox identity 的扫描不会重写或清空当前 deny ACL；最终 ACL setup 前按同一 workspace root 固定所有已存在的精确 protected object：controller 只固定它本来就能访问的对象，sandbox 新建的私有对象由其所有者身份固定，两者都不能证明身份时拒绝执行。目录监视器启动后重新核对这些对象的最终 handle identity，再建立 before snapshot，因此 setup 边界上的普通目录替换、reparse 替换或对象消失都不能被误当作合法空目标。protected path 仅作为不透明的名称/存在集合参与前后比较，内容不会被 snapshot 打开，也不会进入 changed paths、diff digest 或模型；集合变化或 snapshot、impersonation、`RevertToSelf` 任一步无法证明时均 fail closed。child 退出和 Job 清理完成后才执行最终快照；最终 before/after 差异生成规范化相对路径和 SHA-256 diff digest。Windows 目录通知缓冲溢出或查询失败时，若完整前后 snapshot 仍证明 root/protected identity 未漂移且最终状态确实改变，backend 保留物理 `Changed`，但超过精确路径上限时通常不发布 model-facing summary；唯一例外是完整快照证明所有变化均为本轮新建的闭集工具链产物，此时只投影实际发生变化的最小产物根，digest 仍绑定完整 before/after。通知与 snapshot 对最终状态矛盾、protected identity/permission 漂移、root identity 变化或 snapshot 无法复验仍归为 `Unknown`。执行前无法建立快照时返回 `capability_not_supported:workspace_change_summary`，不启动 child；执行后无法复验时 fail closed。缺失、非法 digest 或 edit/patch 请求路径与实际路径不一致会在动态规划前拒绝，不能用 command cwd、模型参数或 call fingerprint 代替真实 diff。只有 producer 证明全部差异都是本轮新生成的闭集工具链产物，且已有祖先目录只发生由这些子项引起的非安全 metadata 波动时，`verification_relevant=false`。缺失字段、非法 digest、非规范/重复/未知路径、preexisting artifact、source/artifact 混合变化或目录身份与权限变化都默认 relevant。workspace-write 下可信的 artifact-only physical `Changed` 仍进入 sandbox event 和 checkpoint，但 `WorkspaceTools` 投影为当前 revision 的 semantic `Unchanged`，避免验证命令自身生成的 evidence 失效；read-only 下任何 physical `Changed` 仍 hard fail。Evaluation 复用同一个闭集分类器，并通过 pristine source snapshot 保留 tracked artifact 路径。成功 verification 同时保存 `Unchanged` observation 和该 revision。任何 semantic `Changed` observation 会清空旧 evidence，多条 exact requirement 只有在所有 observation 仍绑定同一 revision 时才能共同满足。
...
Evaluation 是源码仓库中的开发工具，由独立的 `singularity-evaluation` 二进制读取 `evaluation.task_set/v5` manifest 并执行 AgentLoop runner。产品 `sg`、公共 JSON-RPC 和 `singularity_app_server` 不暴露 Evaluation 命令或方法，Windows release 也不构建或打包该二进制。runner 复用同一 Agent、Tool、Sandbox、Store 和 trace 投影实现，因此评估的是产品能力而不是第二套 runtime；Evaluation 结果不能反向定义 AgentLoop 语义。task set 必须显式声明 1 到 32 的 `trial_count`；每个 task 仍声明用于任务集覆盖审计的非空 `capabilities`，Agent 输入则只声明版本化的 `required_tool_capabilities`。capability name 只受通用 snake_case 语法约束，实际合法性、版本和工具集合完全从当前 `ToolRegistry` 的 `ToolEntry.capability/version/exposure` 解析；Evaluation 不维护工具名或 capability 白名单：
...
Windows command observation 按 canonical workspace identity 与 backend contract 维护有界会话。第一次 command 建立完整 snapshot；`ReadDirectoryChangesW` 连续且变化路径可安全解释时，只重新读取变化对象和必要祖先并更新摘要，同一次创建的新目录及其后代通知归并后只遍历该新子树；同 revision 且无变化时直接复用。最终 snapshot 前启动的稳定性 guard 会作为下一段 session monitor 继续使用；guard 捕获到完整的尾部变化时从刚提交的 snapshot 继续增量更新，不把完整通知降成 `Unknown`。通知溢出、取消、root/contract/protected-path policy 改变、ACL 或 reparse/identity 无法证明时不缓存不完整结果，而是完整重扫或 fail closed。protected path 分为语义规则命中的路径与本次 resolver 明确固定的不透明对象；两者都不会被递归读取。显式集合删除或无法与现有 opaque/public-certificate 事实绑定时使 session 失效；已证明不透明的路径以及重新验证为公共证书的现有文件可以扩展显式集合而不重扫无关子树。

Evaluation 在 `project_agent_task` 完成后、Agent authoritative baseline 前从同一 backend 启动连续 `PreparedWorkspaceObserver`。baseline 后首 checkpoint 必须为 `Unchanged`，否则在 Provider 请求前形成 WorkspacePreparation blocker；observer 随后覆盖整个 AgentLoop。Agent 结束后的首个 checkpoint 只用于绑定 producer summary 与 after snapshot 的读取计划；after snapshot 完成后立即执行第二个 checkpoint，且只有 `Unchanged` 才能提交该快照，`Changed`、`Unknown` 或 checkpoint 错误均 fail closed，防止快照读取窗口中的写入被遗漏。首个 checkpoint 的实际路径必须与每个通过 `WorkspaceChangeSummary::validate()` 的 producer path 形成相同的 ancestor/descendant 闭包：额外路径、缺失 summary、非法/无界路径或 digest、`Unknown`、summary/semantic observation 不配对都 fail closed，不能用一次 Full scan 把 out-of-band 写入计入模型结果。闭包一致时只读取 observer 与 summary 路径的并集及必要父节点；无物理变化才复用 baseline。backend 不提供连续 observer 时保留完整 after snapshot，不伪造增量能力。
...
独立 runner 为本次开发评估创建隔离的 trace Store，每个 run/task/trial 建立显式的 external Task/Turn typed span；Agent 阶段使用 `AgentLoop::run_with_events`，回调事件和最终 provider/cache observations 仍由产品使用的同一个 `TraceProjector` 写入 SQLite。trial 目录中的 `agent-trace.json` 使用 `evaluation.agent-trace/v2`，只由 Store 查询并导出；`AgentLoopResult` 仍用于 result/report 的安全指标投影，但不是 Evaluation trace 权威。Evaluation 的 trace 写入失败会作为 infrastructure blocker 阻止 publication，不会只写文件后继续发布，也不会写入用户的产品会话库。
