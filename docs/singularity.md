...
`ProviderProtocolContract` 把 required tool choice 作为独立于 strict schema、parallel calls 和工具定义容量的能力。OpenAI-compatible adapter 只在固定单工具 probe 实际成立时声明支持；明确拒绝或未履行 required 的成功响应只关闭该可选能力，authentication、rate limit、server、transport、decode 和 cancellation 等 typed failure 不降级为“不支持”。AgentLoop 仅在 contract 支持且本地 completion、plan 或 verification 状态仍不允许 final 时请求 required；能力不成立时保持 `auto`，本地 completion gate 仍 fail closed。
...
9. 没有 tool call 时应用 completion gate，接受或拒绝 final answer；repairable failure 或 completion/plan/verification 状态仍不可 final 时，下一 `ModelTurnRequest` 携带 typed tool result 或固定 developer feedback，并在协商 contract 明确支持时使用 `Required`，否则保持 `Auto`。全部不变量满足后立即回到 `Auto` 以允许 final text。反馈同时列出全部未满足的 plan 与 verification 不变量，不以其中一个遮蔽另一个；普通文本不能绕过本地 completion gate。
...
`builtin.invoke_tool` 是只在工具定义容量不足时投影给模型的 AgentLoop 控制工具。它要求一个非空 `tool_name` 和对象型 `arguments`；动态 schema 由当前 `ToolRegistry` 中每个真实工具的模型输入合同生成，并排除 router 自身。AgentLoop 在同一回合解包后重新执行真实工具的本地 validation、profile binding、整批 preflight、Policy 和 Approval；未知工具、递归调用或非法 envelope 在 executor 前以 typed failure 拒绝。它不持久化成第二份能力、授权状态或执行器。

`AGENT_DEVELOPER_INSTRUCTIONS` 要求多步骤工作使用 `builtin.update_plan` 保持简洁计划，在证据或失败改变路径时更新，并在 final answer 前完成；简单只读或单步工作跳过计划。

## 7. Model 与 provider

`ProviderConfigSnapshot` 在 app-server 启动时只捕获一次配置。进程环境层优先；如果该层完全没有 provider 变量，则从当前目录向上查找最近 `.env`。`SINGULARITY_MODEL`、`SINGULARITY_BASE_URL`、`SINGULARITY_API_KEY` 和 token limits 必须来自同一层，`SINGULARITY_MODEL_PROVIDER` 缺失时使用 `openai_compatible`。context window 默认 128000，output limit 默认 4096；用户不配置 tool-call、tool-definition、required 或 strict 能力。每次 run/resume 都调用 capability negotiation；对同一 `ProviderConfigSnapshot` 与 effective model，已有成功结果时命中 per-effective-model snapshot cache，不发网络 probe，只有 cache miss 才执行固定、无用户数据的 capability probe。OpenAI-compatible adapter 使用固定 developer/user 消息，依次验证产品上限 8 个 direct definitions 下的 strict parallel、strict single、non-strict parallel 和 non-strict single；strict profile 证明 schema 约束，non-strict direct profile 只证明结构化调用形态和数量，不冒充 schema conformance。只有这些 direct profile 都不成立时，才用单个 probe tool 验证包含 8 个真实 router 同形分支的 non-strict envelope。基础 profile 成立后，用同一 transport/parse/retry 路径追加一次固定单工具 required probe；其 usage 和 attempt/latency 进入同一安全 metadata，但不增加 profile attempt 或 fallback 计数。接受超过 8 个定义的上游不会被乐观投影为更大的运行时保证；低容量 contract 也只有 router 参数完整满足 envelope 时才成立，未验证的 system message 和 JSON mode 不进入 contract。基础候选最多 5 个，失败与 usage/attempt metadata 通过既有 single-flight 共享。返回 contract 明确区分 strict、required、parallel、single 和每请求工具定义容量，非法或超出协商 contract 的请求在发送前失败；`Required` 与空工具集合也在本地拒绝，不能在 wire 层静默退回 auto。工具定义容量不足时，AgentLoop 只发送一个 `builtin.invoke_tool` 定义并把每个真实工具的完整模型输入合同嵌入其有界 schema；整个 invocation 在一个模型回合内完成，不建立 provider capability 之外的选择状态。
