...
`ProviderProtocolContract` 将 native structured tools、strict schema、`supports_parallel_tool_calls`、每请求工具定义容量、required tool choice 和 reasoning/history 约束作为独立能力。Provider 只声明自身明确验证过的能力；Agent 的本地 `ToolChoicePolicy.max_tool_calls` 仍按并行能力选择 8 或 1。每次真实 provider attempt 保存开始/结束墙钟、send-to-headers、TTFT、retry/backoff、usage、状态与 model-turn parent；capability-cache Hit/Miss 在实际查找边界记录独立 `observed_at_unix_ms`，因此取消发生在 PromptAssembly 之前时仍可作为 parentless negotiation evidence 挂到 Turn，而不会伪造 Prompt parent 或当前时间。产品只接受 Direct structured tools，定义数超过协商容量时 typed fail closed，不自动切换或按内部阶段隐藏工具。普通工作回合使用 `Auto` 并稳定投影产品注册的五个工具；只有本地 completion evidence 已满足时，AgentLoop 才派生 `tools=[]`、`tool_choice=None` 的 finalization-only 请求。该请求必须返回绑定当前 workspace revision 和终态 command scope digests 的 strict typed Final Review JSON，并让模型结合完整 tool history 判断语义；provider stream 在本地完整缓冲并通过 revision、verification evidence 与 verdict 校验后，只把 `Accept.final_answer` 投影给客户端。无效 envelope、`Reject`、`Repair`、取消或流失败都不泄露原始 JSON 或中间文本；strict review 内容解析失败时，原响应同样不向客户端投影，只能在剩余 model-turn budget 内进入无工具的有界格式纠正。completion evidence、bounded repair 与 review gate 仍由 Agent 本地决定，不把状态机责任转嫁给 provider。
...
- Final Review 是同一状态机的 typed verdict：`Accept` 必须精确绑定当前 revision 和 terminal command scope digests，并结合当前可用的对话和 tool history；`Reject`/`Repair` 阻止 `Completed` 并以有界安全原因回到相同的稳定工具表面。普通工作回合达到上限时，只有最后一个允许的工作响应已经满足 `finalization_ready()` 才允许一次 terminal finalization；其 provider error、取消、空文本或结构化 tool call 仍 fail closed。
