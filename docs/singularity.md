...
`WorkbenchStore` 统一拥有浏览器状态与通知，各视图只订阅自己使用的字段，流式水位变化不重绘任务列表。活动事件采用不可变追加日志，时间线、轨迹与上下文用量只归约新增后缀；`viewPersistence.ts` 负责持久视图的读取、草稿迁移和保存。工具投影保留原始参数、输出和差异，展示标签与格式化文本在组件渲染时生成。
...
稳定历史与活动事件单向归约为 keyed timeline。活动项目原位更新；Session settled 后重读 durable history 并整体替换活动投影。处于底部时，流式更新继续跟随底部；手动展开或收起时保留所操作条目的位置，详情向下展开；用户向上阅读时保留条目锚点，程序恢复位置和内容收缩不改变阅读意图。工具开始事件立即显示工具行；工具名称与 thinking 共用 16px、500 字重及动画机制；工具为蓝色文字，执行中使用白色高光、100° 渐变和 0.7 秒扫光，完成后恢复纯蓝色，不附加“运行中／进行中”文字。thinking 使用蓝粉紫渐变与 1.2 秒扫光，完成后暂停并保留当前位置，重载历史时显示静止渐变；减少动态效果时静止呈现。工具 call/result 以共同 ID 合并为一项；工具直接逐项呈现，不增加批次折叠层；每项的输出与 diff 详情按需展开。最终回答不折叠，消息不提供复制按钮。发送和停止共用粉白动态玻璃球，点击和键盘分别触发对应动作；空闲流动周期4.8秒、光照周期3.6秒，执行或悬停时分别加速至1.4秒和1.05秒，减少动态效果时球体静止。
...
项目文件夹下的会话列表以高度过渡向下展开、向上收回，整体展开/收起360ms；项目列表与全部会话共用条目动画，在240ms内从上方12px向下移动并淡入，按索引间隔50ms，关闭时淡出并随列表收回。菜单仅采用整体透明度淡入，不为菜单项叠加入场动画；工具和思考内容以高度过渡展开收起，历史正文不重复播放入场。整轮执行沿用DSH的“Deep diving...”持续提示，首字等待、工具执行和后续请求等待均保留，15秒后附带耗时；减少动态效果偏好下关闭动画。对话不显示底部请求统计，手动中断只显示 DSH 式安静的“已停止”标记。
...
AgentLoop 的循环为：装配请求、发送流式模型请求、持久化 assistant/tool call、并发执行工具、持久化结果、继续下一步。固定工具是 `read`、`glob`、`grep`、`bash`、`edit`、`write`。单批工具至多 8 个 worker，结果按模型给定顺序回填。同路径 edit/write 使用进程共享的互斥锁，覆盖版本核对、文件替换与新版本记录，跨任务和工具批次生效；观察版本仍由各 Conversation 独立维护。外部进程与 bash 的写入不受此锁约束。
