...
当前 Evaluation runner 尚未接入同一 `SessionStore`：`run_evaluation` 只接收 sandbox、provider snapshot 和 cancellation，Agent 阶段仍调用非事件版 `AgentLoop::run`，并直接把安全字段序列化为 trial 目录中的 `agent-trace.json`。后续实现必须让 evaluation 使用同一 Store（必要时 trusted reopen），用事件版 AgentLoop 写入 Task/Trial/Turn span，再由 Store 查询结果导出 `agent-trace.json`；禁止继续从 `AgentLoopResult` 直接生成第二份 trace 权威。该缺口不影响当前普通 AppServer turn trace 的 SQLite 权威，但在补齐前不能宣称 Evaluation 已统一。
