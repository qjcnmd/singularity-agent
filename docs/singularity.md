...
checkpoint、pending tool call、原始 prompt、provider payload 和内部 audit metadata 不序列化到 `AgentLoopResult`、CLI response 或普通 trace payload。ordinary/approval checkpoint 分别使用 v7/v8，保留 messages、provider reasoning replay、typed occurrences、workspace facts、used grants、usage、attempts、resume cursor、seen/completed fingerprints 和 context trace；删除 completion/repair/recovery 派生字段。旧 v5/v6 只在 decoder migration seam 读取，当前 runtime 不进入旧状态分支。
