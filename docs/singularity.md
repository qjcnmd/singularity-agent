...
2. 协商 provider tool capabilities，按返回 contract 构造 `ModelTurnRequest` 和 builtin tool schema。全部普通工具能放入协商的工具定义容量时直接投影且不暴露 selector；超限时先只暴露 exclusive、无 workspace 副作用的 `builtin.select_tool`，其本地生成的 `oneOf/const` schema 列出真实已注册工具。选择成功后的下一回合仍使用同一 router wire name，但替换为所选工具的完整 schema；adapter 先验证本次 router 名称、JSON envelope 和调用上限，AgentLoop 再唯一映射到选中工具，并在 Policy、Approval 和任何副作用前完成真实 `ToolSpec`、profile、workspace 与整批 preflight。模型不能让 Agent 猜测或静默裁剪工具集合。有界视图和单调用 provider 每个 response 最多一个调用；显式协商多调用能力时，developer instruction 只允许同一 response 放入彼此独立的只读调用，mutation、command、plan、approval-sensitive 或依赖前序结果的调用必须单独提交。
...
8. 执行允许的工具，把 `ToolResult::to_message_payload()` 按原顺序作为 tool message 送回下一模型回合；router 调用在 provider-facing assistant/tool history 中保持实际暴露的 router 名称，payload、内部 ToolResult、Policy、Approval 和执行状态使用唯一映射后的真实工具名称。已选工具在 Policy、Approval 和 executor 前返回 `ToolFailureKind::Input` 时保留同一 route，让下一回合直接按同一完整 schema 修正；成功或其他失败结果清除选择并回到 selector。pending Approval 已保存合法真实工具调用，因此 checkpoint 不复制 selector 权限或选择状态。typed repairable failure 由 completion tracker 和下一回合反馈处理，sandbox/backend/infrastructure/timeout/cancelled 不伪装成普通输入修复。
