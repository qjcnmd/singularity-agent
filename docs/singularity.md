...
- Windows ACL 通过稳定句柄读取和写入，并在 reparse/最终对象身份不确定时返回 typed unsupported/error；真实 ACL 失败会贯穿 setup、restricted-token fallback 和 elevated caller 关闭失败。缺失的受保护元数据目录在 child 启动前物化并保护；若缺失 `.git` 位于已有祖先仓库下则以 typed unsupported 失败，避免改变 Git ancestor discovery。workspace-write command 的现有 protected paths 同时进入 deny-write，未来 secret 文件名不物化。deny-read 状态按 workspace capability SID 归属，在 `Global` 命名跨进程互斥下以原子替换写入并只做单调保护集合合并，不使用共享 Sandbox Users SID 的 last-writer-wins/revoke。
