...
Runner 在任何 Provider trial 前先一次性验证 Provider 配置，再按 manifest 顺序物化全部 task source；只有全部 source 成功才进入 task/trial 循环。CLI 只提供 Feedback 和 Full 两种运行模式。CLI 的 `--max-workers` 只接受 1 或 2；Full 未显式指定时调用 `std::thread::available_parallelism`，可用并行度至少为 2 时选择 2，只有 1 个并行单位或查询失败时选择 1；Feedback 未显式指定时固定为 1，显式值始终覆盖默认选择。设置为 2 时仅不同 task 进入有界动态 worker，source-preparation barrier 仍在首个 Provider 前完成，同一 task 的 trials 以及 public→hidden 仍严格串行。worker 完成顺序不改变 manifest 顺序的 result/report/evidence 投影。远程 source-template cache 在 sandbox preflight 前检查固定模板目录；损坏或不可读条目不降级为 miss，不发起远程 probe、clone 或 Provider 调用，而是在已记录的 sandbox preflight 事实上发布带稳定 `source_cache_*` 码和 task identity 的零采样 blocker。Full 将 Result、Report、Evidence 一起原子发布；Feedback 只写同一 typed Report。任一其他 source preparation blocker 同样保持 `result.tasks` 与 trial 采样为空而 `configured_trial_count` 仍保留；已支持的 sandbox preflight 事实不会被改写成 sandbox blocker。
