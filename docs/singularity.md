...
- Final Review 是同一状态机的 typed verdict：`Accept` 只在 completion evidence、plan 和 repair state 同时满足时产生；`Reject`/`Repair` 会阻止 `Completed`，把安全原因作为下一回合的固定 developer feedback，并由最多 3 次的 `AgentRepairPlan` 驱动重新执行与重验。重复 repair、证据冲突、预算耗尽或取消都在 AgentLoop 内终止为 fail-closed 的 `Failed`/`Cancelled`，不会被普通 final text 覆盖。
