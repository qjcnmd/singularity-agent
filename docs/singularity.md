...
| AgentLoop execution | `AgentKernel -> AgentLoop.run` | Python sidecar only, invoked by `AppServer::run_python_sidecar_if_enabled` | `agent/run` or `agent/resume` over sidecar JSON-RPC | Rust trace summary is appended after sidecar result | Must enter existing Python `KernelBootstrap -> AgentKernel -> AgentLoop` for real model proof | Native Rust AgentLoop unavailable; M1 sidecar turn is synchronous |
...
| Final answer/status | Python `FinalReport` / `AgentLoopResult` | `AgentLoopStatusBridge` -> `TurnStatus` -> CLI renderer | `turn/start`, `turn/status`, `item/agentMessage/delta` | Turn status and `agent_loop_status` updated for completed/blocked/failed/cancelled/running sidecar result | Completed/blocked/failed/cancelled map to durable Rust turn status | `running` and `not_migrated` do not claim completion; no-sidecar path has no assistant delta |
