...
- **crates/runtime**：Thread/Turn 生命周期协调与单轮执行管线，是 Turn 执行的唯一所有者。
  ...
  - [`ThreadCatalog`]：持久化 Thread 目录与只读投影的唯一入口（create/list/resume/rename/archive/read_thread_summary/paged_read），会话目录与进程级写者锁协调器由目录自身持有；目录操作不得绕过本接缝。`Conversation` 不持有线程目录 CRUD。归档拒绝条件分两层且各自拥有、互不合并：跨进程写者（`store::archive_thread` 持锁拒绝 `WriterActive`）、当前绑定会话（TUI UI 策略）。
...
- **依赖方向**：cli → runtime → {core, model, agent, protocol}；agent → {core, model, protocol}——agent 只使用 protocol 的两个持久化共享 DTO（`TurnModelUsage`/`TurnStatus`），不依赖 runtime/UI；protocol 不依赖 runtime/model/agent。
...
protocol 内对 `TurnEvent` 的唯一 params 投影 `turn_event_params`（camelCase；`tool/execution/end` 的 result 包成 `content:[{type:text,text}]`；`provider/attempt` 的可选字段恒出现、无值为 null；`turn/error` 平铺 `threadId/turnId/error`）直接构成 `--json` 事件行的 `params`；协议 golden 测试表逐字钉住。
...
- 摘要调用的 usage 唯一落点为 `CompactionEntry.usage`，经会话累计投影计入总量；强制压缩的重建估算规模经完成回调交付客户端，不落盘。摘要请求走与正常请求同一重试包装（可取消退避、尊重 Retry-After），取消时以 provider cancelled 错误收敛为压缩失败。
