...
> 基于当前源码核对：Rust `crates/core`、`crates/cli`、`crates/app-server`、`crates/protocol`、`crates/model`、`crates/agent`、`crates/tools`、`crates/sandbox`、`crates/store`，以及 Python oracle/parity/dev-only 参考模块。
> `[成功]` / `[失败]` / `[阻断]` 为关键分叉点；缩进表示嵌套层级。

## Rust Agent Host 迁移边界

当前 Rust public runtime 是 Rust `sg` -> app-server JSON-RPC -> Rust AgentLoop。Python 主链路 `CLI -> KernelBootstrap -> AgentGraphBuilder -> AgentKernel -> AgentLoop -> ToolProtocolEngine -> ToolExecutor -> FinalReport` 仅保留为 Python oracle/parity/dev-only 对照；普通 CLI、public protocol、README 和测试文档不能把 Python 描述为可选后端。
...
| AgentLoop execution | `AgentKernel -> AgentLoop.run` for oracle | `AppServer::run_native_agent_loop` is default production path | `turn/start`, `AgentLoopInput`, `AgentLoopResult` | Native appends `agent_loop` trace; project instructions stay memory-only | Native root -> cwd `AGENTS.md` order, developer role, final/tool/failure paths are Rust-tested and require real-provider validation | Python oracle stays fixture/dev-only, not a public backend |
...
| Trace event write | Python `TraceRecorder` for oracle | `SessionStore.append_trace` for Rust/app-server events | `trace/list`, `trace/show`, `trace/tail` | Thread, turn, approval and native summaries persisted | CLI can locate safe event id/component/summary | Raw oracle/provider payload is not projected |
...
| Evaluation proof | Python `EvaluationRunner` for oracle | Rust `sg eval run` native runner | `eval/run` | Rust result/report artifacts stay in configured output dir | `agent_completed`, `tests_passed`, `evaluation_passed`, local process fallback diagnostics stay separate | Python eval is oracle only, not public proof |
...
| Missing thread before native execution | No run status created | No turn | JSON-RPC `Thread not found`, non-zero CLI exit | No native trace |
...
Resume owner: Rust app-server resumes by reading durable Rust thread/turn state and creating a new public turn. Python oracle resume semantics stay inside dev-only parity checks and are not part of public protocol.

Status owner: Rust owns `AgentRunStatus -> TurnStatus -> CLI output -> trace summary` projection for public runtime. Python owns only oracle/parity/dev-only evidence.
...
`accepted` is the app-server boundary after `turn/start` input validation but before Rust AgentLoop has produced a terminal result. Rust owner: `AppServer::turn_start`. Python owner: none. SQLite fields: the current protocol immediately creates `turns.status="running"` and `turns.agent_loop_status="running"` after `SessionStore.create_turn_with_input_and_trace()`. accepted is a lifecycle edge, not a persisted `TurnStatus` variant. trace event: app-server turn started event with redacted input reference only. CLI rendering: running line, no assistant delta. retry/resume implication: safe to retry only if no native run exists.

`running` is the durable status while a Rust AgentLoop turn owns work. Rust owner: `AppServer` plus `SessionStore`. Python owner: none for public runtime. SQLite fields: `turns.status="running"` and `agent_loop_status="running"`. trace event: app-server lifecycle event plus native `agent_loop` trace event. CLI rendering: `turn <id> running ...`. retry/resume implication: do not start a second run for the same turn.

`completed` is terminal success. Rust owner: `AppServer::update_turn_from_run_status` and `SessionStore`. Python owner: none for public runtime. SQLite fields: `turns.status="completed"` and `agent_loop_status="completed"`; if completion is observed during `turn/status`, app-server appends one `AgentMessage` item for the redacted final answer. trace event: safe native completion summary plus one typed agent message delta when a final answer exists. CLI rendering: completed line and bounded assistant summary. retry/resume implication: no retry needed; later work starts a new turn.

`failed` is terminal failure before or during native work. Rust owner: app-server for protocol/store errors and safe status mapping. Python owner: none for public runtime. SQLite fields: `turns.status="failed"` and `agent_loop_status="failed"`. trace event: redacted failure summary. CLI rendering: non-zero failure line without raw prompt/provider/tool/env payload. retry/resume implication: retry must create a new turn or use documented recovery, not overwrite evidence.

`interrupted_requested` records an accepted cancel request while work may still be unwinding. Rust owner: Rust app-server. Python owner: none for public runtime. SQLite fields: current protocol writes `turns.status="interrupted"` and `agent_loop_status="cancel_requested"`; `interrupted_requested` is recorded in lifecycle trace payload. trace event: app-server interrupt requested event. CLI rendering: `turn <id> interrupted agent_loop_status=cancel_requested`. AgentLoop cancel semantics are Rust-owned for public runtime; cancellation must not be implemented by exposing Python as a backend.

`interrupted` is terminal cancellation. Rust owner: `AppServer` and `SessionStore`. Python owner: none for public runtime. SQLite fields: `turns.status="interrupted"` and `agent_loop_status="cancelled"` when the native loop reports cancellation. trace event: safe cancel result event. CLI rendering: `turn <id> interrupted agent_loop_status=cancelled`. retry/resume implication: a future turn starts from durable Rust thread state; the interrupted turn stays terminal.
...
Intentional non-goals: no OS kill as the primary cancel mechanism; no direct CLI-to-Python cancel; no Python backend selection; no local-process sandbox fallback.
...
Use `cancel` for AgentLoop cancel. Use `interrupt` only for the existing JSON-RPC method and CLI command. Use `status` for public protocol output. Use `run` rather than verbose execution names for active work. Use `session` only for recovery identity. Do not introduce verbose invented lifecycle names.
