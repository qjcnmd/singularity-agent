...
For migration reads, `eval run` also accepts retired `evaluation.live_agent_task_set/v1` task-set files and `--baseline-result` can compare against retired `evaluation.live_agent_eval_result/v1` result files. New runs still write only canonical `evaluation.result/v1` artifacts under `work/evaluations/`; the retired `eval live` command and live-named output schema are not restored.
