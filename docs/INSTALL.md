...
超长 bash 输出保存在系统临时目录的 `singularity-tool-output/<uuid>/<命令名>.log`，工具结果会给出完整路径。输出文件继承 Windows 用户临时目录 ACL。新建输出文件时清理超过七天的旧输出；保存失败会显示原因，不提供不完整文件的路径，也不改写命令本身的退出状态。

当前会话格式为 v7，只读取这一格式，不自动迁移旧历史。升级前退出程序，将旧 `sessions/`（含归档）移出；确认不需要旧记录时可以删除，再从空历史开始。配置、凭据、项目登记和浏览器草稿独立保留。
