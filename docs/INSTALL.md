...
可选元数据 `user-invocable: false` 隐藏手动入口，`disable-model-invocation: true` 禁止模型主动调用；两者默认分别为 `true`、`false`。目录在每个 turn 开始及压缩后重新发现，因此同一会话中新加的技能会在下一次输入或压缩后可见。发现时最多读取 64 KiB 的 YAML 元数据；正文及其编码在调用时检查并重新读取，随后附带文件来源与相对资源目录。手动调用的正文随那次输入保留在会话历史中。格式错误会指出文件，不影响其他有效技能；技能中的脚本不会因加载而自动执行。
