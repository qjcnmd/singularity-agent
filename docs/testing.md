...
该 gate 只运行 `docs/evaluation/public-representative-task.json` 中的 `sqlfluff__sqlfluff-2419` 这一项公共 SWE-bench Lite dev 任务；旧真实任务和旧多任务 regression manifest 不属于默认路径。result 中的 `capability_summary.schema_version` 为 `evaluation.capability_summary/v2`，除 model/tool/retrieval/context/compaction/sandbox/verification/finalization 对象流外，还输出 `provider_time_by_turn`、`turn_diagnostics`、逐命令 `sandbox_commands`、`sandbox_breakdown`、互斥 `wall_phases`、`unattributed_time_seconds`、细分 timing 和 `timing_diagnostics`。`sandbox_breakdown` 将 run_verification sandbox path 中的 doctor readiness、ACL grant、workspace low integrity、workspace materialization、spawn、command runtime、output collection、change/artifact collection、cleanup 和 diagnostics overhead 分成真实执行耗时与 diagnostics 观测开销。`turn_diagnostics` 只包含安全 ID、phase/purpose、provider latency、token/cache 计数、tool call、review/verification/finalization 事件状态和耗时；不得包含 prompt、response、文件内容或 evaluator-only metadata。可靠数据不存在时 timing 必须为 `null`，并以 `unavailable` 或 `not_applicable` 说明原因，不能用假 `0` 补齐。

公共任务必须先证明 base commit + evaluator test patch 失败，agent patch 后同一验证通过，且 agent completed、目标文件变化、public/hidden verification、FAIL_TO_PASS、CompletionGate、sandbox enforcement、evaluator visibility audit 全部通过，才允许 `evaluation_passed=true`。sandbox-required 任务要求 `local_process_fallback_count=0`；evaluator-only metadata 不得进入模型可见 task projection 或 AgentLoop trace。没有触发 compaction 时必须写明 skipped reason，例如 context usage 未到阈值、retrieval 内容不足或任务过早完成。result 同时写出 `evaluation_metrics` scorecard，`verify_capability.py` 会把 resolved、tool success rate、cost、pricing 状态、per-turn provider/tool/review 摘要、edit_apply review/critic 明细、`capability_sla_diagnostics` 和 remaining bottlenecks 放入 JSON 输出；这些字段只用于诊断/回归分析，不改变 capability gate 的 pass/fail 判断，也不会因为 cost/pricing unknown、SLA 超限或 timing jitter 诊断导致失败。

Model-assisted review 的输出边界按 Structured Outputs / JSON Schema、strict tool calling with pinned tool choice、`json_mode` fallback、rule-only fallback path 的顺序验证。所有模型输出继续经过本地 schema validation / Pydantic validation；JSON parse error 和 schema validation error 只有 bounded retry，provider transient error 由模型层 bounded retry 和 exponential backoff with jitter 处理，business-rule validation failure 直接 graceful degradation 到 rule-only fallback。strict tool calling path 中，如果 provider 返回了目标 tool，且 `raw_arguments` 本身是可解析 JSON object 并通过同一 Pydantic schema / business-rule validation，review boundary 可以接受该 raw argument projection，避免因内部临时 review tool schema 不在普通 tool registry 中而产生无价值 retry；格式错误、非 object、schema validation failure 仍按 bounded retry 处理。FinalReviewer、CompletionGate、ReviewPipeline、ModelCritic、ReviewDecisionEngine、EvidenceLedger、ReviewFinding 和 ReviewReport 是 Singularity 内部对象名；它们保持 fail-closed deterministic gate / hard gate 语义，不能被模型结果覆盖。capability_summary 的 review diagnostics 只允许 `output_mode`、`schema_validation_passed`、`retry_count`、`fallback_reason`、`model_critic_status` 和耗时/复用状态，不允许 raw prompt、raw response、secret、evaluator-only metadata、raw patch text 或未脱敏文件内容。

`capability_sla` 是 Phase 8.5 起的非阻断诊断层，写入 task result、summary、report 和 `verify_capability.py` JSON 输出，但不改变 `evaluation_passed`、`tests_passed` 或脚本退出码。当前目标为 wall <= 300s、AgentLoop <= 210s、provider <= 55s、sandbox <= 50s、dependency setup <= 35s；verification、unattributed time、local fallback 和 visibility audit 也会显示在 SLA 表中。SLA 超限必须报告 `over_sla` 和 delta，但不得通过减少 public/hidden verification、放宽 CompletionGate、扩大 ACL、绕过 Windows sandbox 或引入 local process fallback 来压时间。dependency setup 和 sandbox readiness 的优化只允许使用安全、可审计、可失效的缓存或现有 provider/native cache 诊断；Windows sandbox backend 会在同一 backend 实例内复用短生命周期 readiness snapshot，run 前仅在证据缺失、过期、不可用或网络隔离证据不足时重查，仍保持 fail-closed。当前 runner 对 evaluator-only prepare commands 注入 `PIP_CACHE_DIR`，并可在命令目标为 `../.eval-venv` 时复用 keyed prepared evaluator venv。`reproducible_environment.dependency_setup_cache.schema_version="evaluation.dependency_setup_cache/v2"` 记录 key、hit/miss、pip cache、wheelhouse、prepared env、失效输入和安全边界；`verify_capability.py` 还输出同 task/start_commit repeated runs 的 min/median/current timing compare，并基于 current SLA item 加 `evaluation.capability_sla_diagnostics/v1`。当 current run 轻微超标且 `median <= target` 时诊断为 `timing_jitter`，当 median 也超标时诊断为 `persistent_over_sla`；这些诊断只解释 remaining bottlenecks，不改变 `capability_sla.status` 或 hard gate。
