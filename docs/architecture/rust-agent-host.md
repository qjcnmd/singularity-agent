...
`crates/app-server` 实现 `initialize` / `initialized` handshake、server capabilities、thread list/read/start/resume/fork/archive/delete、turn start/interrupt/status、event subscription、approval list/center/request/decision、artifact fetch、trace list/show/tail 和 `server/shutdown`。默认 `turn/start` 明确写入 `agent_loop_status = "not_migrated"`，不伪装 Python AgentLoop 已完成迁移；`turn/start` 会先校验 Rust store 中的 thread 存在，missing thread 直接返回 `Thread not found`，不会启动 Python sidecar 或写 sidecar/native trace。显式 `agentHost="native"` 先检查 `AgentLoopCapability::current()`；只要 `available=false` 或 `missing_boundaries` 非空，app-server 返回 `Native AgentLoop is not production-ready`，不会创建 durable turn、不会写 native trace，也不会调用 Python sidecar。未来 capability gate 打开后，`AppServer::run_native_agent_loop()` 会用 `OpenAiProvider::from_env()`、`ToolBroker`、`PolicyEngine` 和 `WorkspaceTools` 执行 Rust `AgentLoop`，并把终态写回 turn 与 `component="agent_loop"` 的安全 trace；该路径不调用 Python sidecar，不注册 command tool，不提供本地进程 fallback。显式设置 `SINGULARITY_PYTHON_SIDECAR=1` 且 thread 校验通过后，app-server 先持久化 Rust turn/user item/turn trace，再通过 `PythonSidecarClient` 启动 `python -m singularity.agent_host.sidecar`；首个 sidecar turn 调 `agent/run`，后续 turn 通过上一条 `python_sidecar` trace 的 `session_id` 调 `agent/resume`，thread 的 `model` 会转发为 sidecar `model` 参数。sidecar 调用现有 `AgentHost -> KernelBootstrap -> AgentKernel -> AgentLoop`；Python 仍拥有 sidecar oracle 的 AgentLoop 语义，Rust 只把安全状态摘要翻译成 turn/item/trace。sidecar 返回 `running` 时，app-server 写 active run row 并保留当前进程内 sidecar handle；持有该 handle 的进程内 `turn/status` 通过 `agent/status` 刷新 durable status，`turn/interrupt` 通过 `PythonSidecarClient::cancel(run_id)` 请求 cancel 并写 `agent_loop_status="cancel_requested"`。如果另一个短生命周期 app-server 进程只看到 active run row 但没有 sidecar handle，`turn/status` / `turn/interrupt` 只返回 durable turn status 和 active row status，不伪造 sidecar 查询或取消。`server/shutdown` 会清理当前进程持有的 active run，设置 shutdown_requested，并让 binary 主循环在写出响应后退出；CLI drop 会先发送 `server/shutdown`，再等待短窗口，只有 app-server 未退出时才 kill child；app-server drop 或 stdio 进程退出时也会写 cleanup lifecycle event 并清 active run row。`thread/delete` 会先清理同一 thread 的 active run，再删除 thread/turn/item/trace/artifact row。未取消的 active run 标记为 failed，已经 cancel_requested / cancelled / interrupted 的 run 保持 interrupted 并投影为 `agent_loop_status="cancelled"`，避免 SQLite 永久保留 running row 或把已取消 turn 误标失败。stdio binary transport 的错误行统一通过 `JsonRpcMessage::error()` / `serde_json` 序列化为合法 JSON-RPC error envelope。item streaming 使用 `item/agentMessage/delta` 和 `item/commandExecution/outputDelta` 这类 typed delta，不再使用 generic `item/delta`；completed sidecar final answer 只作为 redacted agent message delta 投影一次，未来 native gate 打开后的 completed final answer 也必须遵守同一投影规则；默认 no-sidecar path、native gate rejection、blocked/failed/cancelled sidecar/native failure 和 sidecar transport error 不输出伪 assistant delta。当前 `server/capabilities` 只声明 stdio transport 可用，WebSocket token transport 仍是未来 transport，显式返回 unavailable。
...
`crates/agent` 当前除 Python sidecar bridge 外，持有 M9 的 schema/parity 切片 `PlannerStateBoundary`、`ContextAssemblyBoundary`、`ContextSummaryEnvelopeBoundary`、`ToolCallRepairBoundary`、`FinalizationMappingBoundary` 和 `AgentLoopCapability`，并提供小型 deterministic helper：`assemble_context_items()`、`planner_next_action()`、`repair_next_action()`、`completion_gate_allows_final()` 和 `final_mapping_from_status()`。`AgentLoop` 是当前显式 native 最小循环：context item -> provider request -> provider response -> final-answer gate 或 tool admission -> safe workspace tool observation -> max-turn fail-closed。completion gate 未迁移前，模型直接给 final answer 会返回 blocked / `completion_gate_not_migrated`，不会标记 completed；denied/unknown/ask tool 不执行，denied/unknown tool failure 会立即 fail closed，不能被后续模型轮次转换成成功。`.env`、`.ssh`、`.git`、credential/secret/private-key path 会先标记 sensitive 并 fail closed。`AgentLoopCapability::current()` 仍返回 unavailable / `not_migrated`，并列出 `planner_step`、`compaction_executor`、`tool_repair_runtime`、`completion_gate`、`strict_command_sandbox` 和 `rust_evaluation_runner`；这表示最小 native loop 只能由 agent crate 测试覆盖，app-server 和 CLI 在 blocker 清空前都不能把它暴露为 production capability。完整 context assembler、repair planner runtime、finalizer runtime、strict command sandbox 和 Rust eval runner 仍是后续独立切片，不能和当前切片一起重写。
...
如果 provider 配置存在，还要通过当前 Python 路径运行真实 AgentLoop evaluation，并验证 Rust CLI 和 app-server native gate 都会 fail closed。最小 native loop 由 Rust agent tests 覆盖；在 `AgentLoopCapability::current()` 的 blocker 清空前，`sg run --agent-host native` 和直接 JSON-RPC `agentHost="native"` 都必须被拒绝，而不是把最小 native loop 当成 production proof。Rust eval 仍必须报告 `rust_evaluation_runner_not_migrated`，default `sg run` 仍不能切到 native；remaining capability blocker 继续包含 `strict_command_sandbox` / `rust_evaluation_runner`：
...
1. strict sandbox backend execution contract；当前 `crates/sandbox` 只保留 command request/result、backend descriptor、`PatchChange` / `PatchResult` schema contract 和 capability schema，不公开 relaxed local-process executor 或 host filesystem mutation executor。
2. planner step、context assembler、compaction executor、tool repair runtime、completion gate 和 finalizer runtime 继续按一个行为切片一组 fixture 推进。
3. Rust evaluation runner 最小公共 representative task，必须保持 `agent_completed`、`tests_passed`、`evaluation_passed` 和 local_process fallback 诊断分离。
5. 更完整的 TUI 渲染和长期 app-server lifecycle 管理。
6. native capability blockers 清空且真实 eval 通过后，才能把默认 `sg run` 切到 native。
...
M0 之后，`scripts/verify_rust_migration_boundaries.py` 是提交和 CI 的迁移漂移检查入口。它检查 `crates/cli` 不能直接依赖 agent/model/tools/store，crate 依赖必须留在显式 allowlist，Python runtime 改动只能落在 sidecar/oracle/fixture/parity 允许路径，仓库不能提前出现 desktop/Web 启动文件，默认非 native `turn/start` 必须保持 `agent_loop_status = "not_migrated"`，native production capability 必须保持 unavailable 并列出 strict command sandbox 和 Rust eval blocker，CLI 和 app-server 必须在 native path 同时检查 native capability `available` 和 `missing_boundaries` 为空，blocker 未清空前不能发送或执行 native `turn/start`；`crates/sandbox` 不得公开 relaxed/no-sandbox contract 或 local-process executor，app-server stdio error 不得手拼 JSON，CLI 不得恢复固定 notification 等待或 post-response drain，store 不得恢复重复 approval decision public API，CLI/app-server 不得声明 unused tokio，并且 Rust `ToolObservation.to_model_payload()` 不得输出 raw arguments、内部 approval/policy id、internal metadata 或明显 secret-like 文本。Python runtime allowlist 只覆盖 sidecar/oracle/fixture/parity 路径，以及 evaluation 仍依赖 Python AgentLoop 时 `src/singularity/model/runner.py` 的 context-export fail-closed 策略修复和对应 `tests/test_model_runner.py` 回归；不允许在 Python 主干新增 agent runtime 能力。
