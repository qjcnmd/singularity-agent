...
`crates/policy` 提供 Rust `PolicyEngine` 纯决策边界。`PermissionRule` 按 `SettingsScope` 的 managed、user、project、local 顺序匹配，并按 hooks -> deny rules -> protected resource deny -> defer rules -> ask rules -> allow rules -> fallback ask 的顺序得到 `PermissionDecision`。默认保护 `.env`、SSH key、credential、secret、token 等 resource marker；`approval_policy = never` 会把 ask 转成 deny。当前 Rust policy 只消费调用方已经归一化好的 `PermissionRequest.resource`，不解析 shell wrapper、argv、workspace path、network host 或命令等价形式；这些上下文归一化与 enforcement 属于 command/sandbox 或上层 tool boundary。当前 Rust policy 仍是 app-server/tool boundary 的纯决策对象，完整运行时 policy audit writer 和 Python AgentLoop 执行强制链尚未被 Rust 替换。
