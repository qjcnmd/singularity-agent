# Agent Kernel / Session Lifecycle Component

Singularity now has a component kernel layer in `src/singularity/kernel/`. The kernel is the process-level control plane: it owns boot, agent graph assembly, run/session lifecycle, workspace locking, cancellation, shutdown, crash recovery, health checks, and final report aggregation. Planner remains focused on task planning and completion evidence; it no longer owns system lifecycle.
...
```text
CLI -> KernelBootstrap -> AgentKernel -> AgentLoop -> Planner
```

`src/singularity/cli.py` still parses user flags into `ProductionConfig`, but component construction is delegated to `KernelBootstrap`. The CLI then calls `AgentKernel.run_task()` and prints both the model final answer and the kernel `FinalReport`.
...
The full component config precedence is:

```txt
explicit CLI flag > SINGULARITY_* > .singularity/config.toml > defaults
```

`ProductionConfig.effective_config()` returns the redacted effective values and source map. `KernelBootstrap.boot()` records that payload in trace with component `config`, and kernel final reports include the same source-aware summary. `SINGULARITY_API_KEY` remains environment-only and is not written to the effective config payload.

## AgentGraph

`AgentGraphBuilder` builds a `AgentGraph` in this declared order:
...
Each component is recorded as `component.initialized` in trace when its boot-time boundary is ready. `AgentGraphBuilder.build()` assembles the graph in explicit phases: infra, policy/sandbox, execution core, tools/plugins/protocol, verification/review, model/context, then planner wiring. The plugin boundary runs after built-in tools are registered and before `ToolExecutor` / `ToolProtocolEngine` are created, so enabled local tool plugins become ordinary `ToolSpec` entries and still execute through the same policy, approval, sandbox, trace, and protocol layers. The evaluation boundary is registered during boot but the full `EvaluationHarness` object is created only when evaluation functionality is actually accessed, so normal agent runs do not pay for benchmark scoring, replay, and artifact-writer setup. The graph creates `ContextManager` before Planner, then `AgentKernel` passes that context into `AgentLoop`. Command, Mutation, Tool, Review, Evaluation, and Verification components are wired back to the session Planner after Planner creation so execution evidence still lands in the existing planner ledger.
...
`CancellationManager` owns a root `CancellationToken` and child tokens. `AgentGraph` owns the list of cancellation-aware component targets, and `AgentKernel` asks the graph to attach child tokens so downstream layers can honor cancellation without owning process shutdown. Lazy evaluation construction uses the same graph-level token factory when evaluation is accessed after boot.
...
`ComponentHealthChecker` checks the agent graph components, including plugins and deferred components such as evaluation, without forcing lazy components to instantiate. Missing components become diagnostics; critical missing components fail closed. Results are written to trace as `component.health_checked` and included in `FinalReport.component_health_summary`.
...
This layer is ready to support later daemon, TUI, remote attach, external supervisor, and multi-session capabilities because component construction and process lifecycle now sit above Planner instead of inside the CLI command body.
