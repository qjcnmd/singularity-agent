...
Singularity routes agent-owned workspace edits through a mutation manager instead of raw filesystem writes. Phase 1B adds a safe model-facing `write_file` facade, but that facade still delegates through `EditExecutor`, `WorkspaceMutationManager`, `WorkspacePathResolver`, and `AtomicWriter`. The goal is to make every file change explicit, reviewable, auditable, reversible, and safe against path escape or stale snapshots.

The component is intentionally compact, but it establishes the production boundary:
...
Tool handlers are not allowed to open, write, remove, or rename files directly. `ToolExecutor` rejects any `WRITE` tool unless its `ToolSpec` declares `uses_mutation_manager=true`.
...
The component implements the safe text operations plus `ApplyUnifiedDiff` for text create/modify patches, `CreateFile`, `DeleteFile`, `MoveFile`, and `UpdateJson`. Parser-backed operations that are not implemented yet return `invalid_operation` instead of writing through an unsafe fallback.
...
`src/singularity/workspace/mutation_manager.py`

`WorkspaceMutationManager` builds and applies `ChangeSet` objects. A changeset contains:
...
They return compact observations containing mutation status, changed files, diff summary, risk note, and next recommended action. The default model-facing write tools are the Phase 1B facades `write_file` and `apply_patch`, registered with the edit tools and delegated into this component. `inspect_diff` reads the in-memory changeset ledger for current-run diff evidence without requiring Git.

## Apply Flow

1. ToolExecutor validates tool arguments with Pydantic.
2. ToolExecutor checks tool policy and rejects unsafe write handlers.
3. The mutation tool builds edit operations and calls `WorkspaceMutationManager`.
...
6. The component builds final file content in memory.
...
11. After each write, the component records the after snapshot.
...
The component records git state before and after apply:
...
`WorkspaceMutationManager` accepts a verification hook and carries `verification_status` in results and trace. The component does not hard-code formatting, linting, type checking, testing, or builds. Those belong to VerificationRunner, which receives changed-file, transaction, and changeset context and then runs checks through CommandExecutor.

## Error Taxonomy

The component returns structured error codes:
...
Not every reserved code is emitted by the current compact implementation, but the names are part of the execution contract and test surface.
...
ToolExecutor rejection of write handlers that bypass WorkspaceMutationManager
registered mutation tool application through ToolExecutor
```
