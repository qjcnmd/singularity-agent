...
公共代表性 capability task 以 `workspace-write`、`approval_policy=never`、`network_access=allowed` 进入真实 `KernelBootstrap -> AgentGraphBuilder -> AgentKernel -> AgentLoop.run` 路径；当前 Rust command backend 不承诺 OS 级网络隔离，显式 `denied` / `allowlist` 会在 Rust command boundary fail closed。AgentLoop 内的低风险验证命令由 policy 判为 `sandbox_required` 后必须进入 `CommandExecutor -> SandboxManager`，Windows 上先选 `windows_elevated`，elevated 因本机 blocker 不可用时可降级 `windows_unelevated`。`sandbox_required` 是沙箱强制执行信号，不计入 evaluation 层的 `policy_blocks`；`policy_blocks` 只统计真实 deny 或扣除 sandbox-required 后仍存在的 policy skip。只有 elevated 与 unelevated 都不可用，或 final report 的 failure repair summary 报告 Python `_ssl` / OpenSSL runtime `environment_error` 时，runner 才在 post-agent verification 前归约为 `environment_blocker`，保留模型、工具、patch 与 trace 证据，但不把普通本地进程或 post-agent verification 当作 AgentLoop 成功。manifest 的 `smoke_command` 进入 Planner 后会成为唯一 AgentLoop smoke command；benchmark constraint 存在时，VerificationRunner 不接受模型在 tool 参数中追加其他 smoke command。当该 manifest smoke 与自动 Python syntax check 都是 Python-like executable、精确 `-m py_compile` 且规范化文件参数完全一致时，只保留显式 smoke；文件子集、pytest、`-c` 或不同命令不去重。公共 task 的 `evaluation_passed=true` 还要求 sandbox enforcement 证明 `local_process_fallback_count=0`，`windows_unelevated` 作为 reduced backend 写入 `reduced_backend_count` / `reduced_backends` 诊断但不等同于 local process fallback，也不冒充 `windows_elevated`；同时要求发送给模型的 task payload 与 AgentLoop trace 的 evaluator visibility audit 可审计且无 `fixture_metadata`、`hidden_test_patch`、`test_patch` 泄漏。tool call、verification evidence、failure repair、completion gate 和 sandbox fail-closed 等内部链路由 deterministic tests 覆盖，不再通过旧多任务真实 provider manifest 保持默认 coverage。
...
- `EvaluationRunner.run_task()` 消费 `EvaluationTask`。`_task_goal()` 只把 `user_task`、`allowed_paths`、`tool_policy`、`allowed_tools`、`expected_file_changes`、`completion_standard`、`risk_tags` 和公开 `smoke_command` 选出的 local smoke 写入用户 goal；hidden command、verification prepare command、`success`、`description`、`fixture_metadata` 与 `hidden_test_patch` 不进入模型请求。`smoke_command` 只允许承载简洁用户可运行检查；不得承载 evaluator setup、gold patch、test patch、FAIL_TO_PASS 或 `.eval-venv` 路径。对需要 baseline fail 检查的公共任务，`_task_goal()` 与 `_apply_benchmark_constraints()` 只发送显式声明的 local smoke；未声明 local smoke 时不发送 evaluator verification command。`_apply_benchmark_constraints()` 发送 task id、allowed tools、expected changes、completion standard、risk tags 和安全的 smoke command。`EvaluationTaskSet` 本身不进模型，只由 `EvaluationRunner.run()` 遍历。
