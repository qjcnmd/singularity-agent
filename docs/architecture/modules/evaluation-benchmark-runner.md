...
review diagnostics 只投影 model-assisted review 的安全 output boundary 字段：`output_mode`、`schema_validation_passed`、`retry_count`、`fallback_reason`、`model_critic_status` 和耗时/复用状态。`output_mode` 的值来自 Structured Outputs / JSON Schema、strict tool calling with pinned tool choice、`json_mode` 或 rule-only fallback path。schema validation 失败只允许 bounded retry；provider transient error 的 bounded retry 与 exponential backoff with jitter 在模型层完成；business-rule validation failure 直接拒绝模型结果并记录 fallback path。FinalReviewer 与 CompletionGate 仍是 fail-closed hard gate：模型辅助审查只能确认已有安全 evidence ref，不能覆盖 failed evidence、sandbox enforcement、evaluator visibility audit、public/hidden verification 或 FAIL_TO_PASS。capability_summary 和 `scripts/verify_capability.py` 不记录 raw prompt、raw response、secret、evaluator-only metadata、raw patch text 或未脱敏文件内容。
