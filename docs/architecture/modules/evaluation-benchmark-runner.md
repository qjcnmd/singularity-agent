...
- `EvaluationWorkspace` 由 `manifests.py` 中的 `EvaluationWorkspace.from_dict()` 解析 manifest；private benchmark 由 `SingularityPrivateBenchmarkAdapter._convert()` 构造。`EvaluationTask.from_dict()` 组合 workspace 与 task 配置，`EvaluationTaskSet.from_dict()` 再组合完整 task set；`load_evaluation_task_set()` 负责读取 JSON 并设置 `base_dir`。
