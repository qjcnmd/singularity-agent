...
`docs/evaluation/legacy/internal-smoke-regression-tasks.json` 中三个pytest类fixture（`simple_patch`、`multi_file_reasoning`、`failure_repair`）在fixture workspace内显式包含`pytest.ini`，内容为`addopts = -p no:anyio`。该 legacy manifest 仅用于 manual/debug/internal smoke，不再作为默认真实 provider capability gate；生产项目自身pytest插件行为和`completion_gate` fixture不受影响。
