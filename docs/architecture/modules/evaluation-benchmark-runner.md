...
`load_evaluation_task_set()` 从 manifest JSON 生成 `EvaluationTaskSet`/`EvaluationTask`。`EvaluationRunner.run()` 遍历 task，每个 task 调用 `KernelBootstrap.boot()` -> `AgentKernel.run_task()` -> `AgentLoop.run()` 执行真实 agent。runner 读取 trace summary、final report、changed files 和 public/hidden verification，由 `_task_result()` 聚合生成 `EvaluationTaskResult`、嵌套 `capability_sla.schema_version="evaluation.capability_sla/v1"` 和嵌套 `evaluation_metrics.schema_version="evaluation.metrics/v1"`。结果写入 `<run_dir>/result.json`、`report.json`、`report.md`。canonical 完成/判定字段为 `agent_completed`、`evaluation_passed`、`miscompletion_count`，不是 manifest 的 `success`；`capability_sla.blocking=false`，只报告 wall、AgentLoop、provider、sandbox、dependency setup、verification、unattributed time、local fallback 和 visibility audit 的目标/实际/delta，不改变 `evaluation_passed`、`tests_passed`、`agent_completed` 或 capability gate 退出码；`reproducible_environment.dependency_setup_cache.schema_version="evaluation.dependency_setup_cache/v1"` 记录 prepare-only pip cache 的 scope、key、cache dir、失效输入和安全边界；`evaluation_metrics.resolved.value` 只是 `evaluation_passed` 的诊断投影，不改变硬通过语义。
