...
- `EvaluationTaskResult` 只由 `EvaluationRunner._task_result()` 聚合 AgentLoop 结果、public/hidden verification、patch、scope、token/cache、trace 与 reproducible environment 生成；`capability_sla` 和 `evaluation_metrics` 也在这里由纯函数从同一批 summary/evaluator/report/trace/token 字段派生。
...
- `summarize_evaluation_results()`、regression compare/report、`FailureCaseReplayRunner` 和 CLI JSON 输出消费 `EvaluationTaskResult`。summary 会额外聚合 `resolved_count`、`resolved_rate`、FAIL_TO_PASS/PASS_TO_PASS 计数、平均 tool success rate、总 cost、cost per resolved、miscompletion、failure reasons 和 `capability_sla.schema_version="evaluation.capability_sla_summary/v1"`；这些都是诊断/回归分析字段，不参与 gate pass/fail。`scripts/verify_capability.py` 读取 `capability_summary.turn_diagnostics`、review events、`provider_latency_by_review_stage`、`sandbox_breakdown`、`capability_sla.items` 和同 task/start_commit 的历史 result，在 JSON 输出中展示 turn/provider/tool/review 摘要、review-stage provider latency、edit_apply review/critic 明细、repeated timing min/median/current、`evaluation.capability_sla_diagnostics/v1` 和剩余 over-SLA 瓶颈；`capability_sla_diagnostics` 只把 current run over-SLA 项解释为 `timing_jitter`、`persistent_over_sla`、`current_run_over_sla` 或 `unavailable`，不改写 `capability_sla.status`。该脚本只改变本地诊断输出，不改变 `EvaluationRunner` 的判定。`TargetedFailureReplayResult.to_dict()`、Markdown renderer 与 `eval targeted-replay` 的退出码逻辑消费 targeted replay result；两种 result 都不再进入模型。
...
- `EvaluationRunner.run()` 在 `<output_root>/<run_id>/` 写 `result.json`、`report.json`、`report.md`；有 baseline 时写 `regression.json`、`regression.md`，失败样本写 `failure_cases.json`。默认 `output_root` 是 `work/evaluations`。`capability_summary` 随每个 `EvaluationTaskResult` 落盘，schema 为 `evaluation.capability_summary/v2`，包含 model/tool/context/sandbox/finalization 计数、`provider_time_by_turn`、`provider_latency_by_review_stage`、`turn_diagnostics`、按 `command_id` 的 `sandbox_commands`、`sandbox_breakdown`、互斥 `wall_phases`、`unattributed_time_seconds`、timing 子对象和每项 `timing_diagnostics`；`provider_latency_by_review_stage` 只按 review stage 聚合 call count、failed call count、total seconds 和 max seconds，不保存 prompt/response。`sandbox_breakdown.items` 把 doctor readiness、ACL grant、workspace materialization、process spawn、command runtime、output collection、change detection、artifact collection、cleanup 和 diagnostics overhead 标注为 `actual_execution` 或 `diagnostic_observation`。`turn_diagnostics` 中每个 turn 只保留 request/phase/action/purpose、provider duration、token/cache 数值、上下文计数、tool call id/name/status/duration、review stage/decision/status/duration、`retry_reason`、verification check status/duration 和 finalization event status。同一 timing 子对象也提升为 `EvaluationTaskResult.timing`，供本地 gate 脚本做 null-aware 聚合。`capability_sla` 随 task 落盘，schema 为 `evaluation.capability_sla/v1`，summary 中另写 `evaluation.capability_sla_summary/v1`；每项都包含 actual、target、status、delta、source 和 `blocking=false`。`reproducible_environment.dependency_setup_cache` 随 task 落盘，缓存 root 默认在 `work/evaluations-dependency-cache/`，只服务 evaluator prepare commands 的 pip wheel/http cache、wheelhouse 和 prepared venv，不进入模型 goal、planner constraints 或 verification command。sandbox terminal event 对同一 `command_id` 只累计一次；不同 command id 即使 argv 相同仍分别累计；有 command lifecycle 时不再叠加同一 sandbox terminal duration。公共任务额外落盘 `baseline_checks`、`baseline_failed`、`patch_applied`、`fail_to_pass_satisfied`、`sandbox_enforcement_passed`、`evaluator_visibility_audit_passed` 和 `verification_misconfiguration_reason`。`evaluation_metrics` 随 task 落盘，内部 `cost` 优先使用 trace provider usage 的 `cost_estimate`，否则仅按 token usage、精确匹配 model pricing table 和 base URL 计算；未知模型或未定价时输出 unknown，不让 capability gate 失败。
...
`CommandEvalResult.to_dict()` 额外输出派生字段 `passed`，`TargetedFailureReplayResult.to_dict()` 额外输出 `schema_version`，这些不是 dataclass 字段；维护字段校验时必须区分“源码字段完整性”和“序列化派生字段”。`EvaluationWorkspace.kind` 序列化为 `type`，也是明确投影而非 alias。`fixture_metadata` 和 `hidden_test_patch` 可以落盘到 manifest/task对象，但禁止进入模型 goal、planner constraints、trace model request payload 或 raw model artifact。Task prepare commands 可使用 runner 注入的 `PIP_CACHE_DIR`、wheelhouse 和 prepared evaluator venv cache，但该 cache 只能是显式审计、可删除失效的 dependency setup 加速，不能改变 task 可见 verification、扩大 sandbox ACL、污染 workspace 或绕过 Windows sandbox。`TaskExecutionEvidence` 仍是 dict-heavy report evidence；新增 evidence 名称时必须接入真实 verification、trace metrics、hook results 或 diff/assertion 来源，不能只在 schema 中登记。`capability_sla`、`sandbox_breakdown`、`turn_diagnostics`、review diagnostics 与 `evaluation_metrics` 都是诊断层，不能反向改变 `evaluation_passed`、`tests_passed`、`agent_completed`、`status` 或 capability gate 退出码。ReviewPipeline、ModelCritic、ReviewDecisionEngine、FinalReviewer、CompletionGate、EvidenceLedger、ReviewFinding 和 ReviewReport 是 Singularity 项目内部对象名；描述输出边界、重试和降级行为时使用 Structured Outputs、tool calling、tool choice、JSON Schema、schema validation、bounded retry、exponential backoff with jitter、fallback path、graceful degradation、fail-closed、deterministic gate、hard gate 和 model-assisted review 等主流术语。
