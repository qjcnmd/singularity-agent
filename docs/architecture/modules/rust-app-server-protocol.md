...
| Final answer/status | Python `AgentLoopResult` / `FinalReport` | `AgentLoopStatusBridge` and `AppServer::update_turn_from_bridge` | `turn/start`, `turn/status`, typed item delta | Turn status and `agent_loop_status` update; safe sidecar trace | Status maps are explicit and testable | `running/not_migrated` is not completion; no-sidecar path has no assistant delta |
