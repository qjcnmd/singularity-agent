...
### M9 planner state parity contract

该对象属于 Rust `crates/agent` 的 schema/parity boundary，不属于 app-server runtime execution。它只镜像 Python oracle fixture 中已经存在的 planner state 安全字段，用于 JSON roundtrip 和 schema generation。默认 `turn/start` 仍使用 `AgentLoopStatusBridge::not_migrated()`。context bundle、compaction summary、repair verification contract、execution outcome 和 finalization mapping 不在当前切片内。
