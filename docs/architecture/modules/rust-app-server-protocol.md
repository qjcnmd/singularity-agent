...
- NativeAgentLoopPlan: steps, merge_requirements
- NativeAgentLoopInput: thread_id, turn_id
...
这些对象属于 Rust `crates/agent` 的 schema/parity boundary，不属于 app-server runtime execution。它们镜像 Python oracle fixture 中已经存在的 planner state、context bundle、context summary envelope、tool-call repair contract 与 finalization mapping 安全字段，用于 JSON roundtrip 和 schema generation；A6 还增加 `assemble_context_items()`、`planner_next_action()`、`repair_next_action()`、`completion_gate_allows_final()` 和 `final_mapping_from_status()`，只做安全字段过滤、预算收敛、next action 和 status 映射。`NativeAgentLoopCapability::current()` 仍声明 Rust native AgentLoop 不可用，并列出 provider adapter、tool runtime、planner step、context assembler、compaction executor、tool repair runtime、completion gate 和 finalizer runtime 尚未完整迁移。默认 `turn/start` 仍使用 `AgentLoopStatusBridge::not_migrated()`。repair planner runtime、finalizer runtime、execution outcome 和完整 AgentLoop 不在当前切片内。
