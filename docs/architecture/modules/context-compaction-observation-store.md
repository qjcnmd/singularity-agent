...
该层负责上下文快照、压缩摘要、工具结果记录和 crash recovery 所需的最近状态，防止长任务上下文无限增长。
...
tool result context record 的 sensitivity 使用 `ContextSensitivity` 枚举（与 context-assembly-prompt-frame 模块共享）：
