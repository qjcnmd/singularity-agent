...
Rust AgentLoop 层负责把一个已创建的 turn 转换成真实模型请求、工具调用、approval request、tool repair、最终答案和安全状态摘要。app-server 在进入主循环前由 Rust `crates/core` 从最近 Git workspace root 到 thread 实际 `cwd` 逐层读取 `AGENTS.md`，父级先、子级后，并把合并内容作为 developer message 放在 user message 之前。它是当前 public runtime 的主循环；Python AgentLoop 只保留为 oracle/parity/dev-only 参考，不是普通 CLI、app-server 或 evaluation 的执行后端。
...
普通 turn：`sg run` / `sg chat` / `sg continue` -> Rust CLI JSON-RPC `turn/start` -> `AppServer::turn_start()` -> `native_capability_ready()` -> `SessionStore.create_turn_with_input_and_trace()` -> `AppServer::run_native_agent_loop()` -> `OpenAiProvider::from_env()` -> `load_project_instructions_from_cwd()` -> `native_loop_input()` -> `AgentLoop::new()` -> `AgentLoop::run()` -> `AgentLoopResult::to_run_status()` -> `AppServer::append_native_trace()` -> `SessionStore.update_turn_state()` / `SessionStore.append_item()`。

Evaluation：`sg eval run` -> Rust CLI JSON-RPC `eval/run` -> app-server native eval runner -> 准备 task workspace -> `load_project_instructions_from_cwd()` -> per task `AgentLoop::new()` -> `AgentLoop::run()` -> smoke command / public verification / hidden verification -> result/report artifact 写入 `work/evaluations/<run-id>/`。

## 真实任务中的对象流

以用户要求修复 `quicksort.py` 为例：`AppServer.turn_start()` -> `AgentLoopCapability.current()` -> `SessionStore.create_turn_with_input_and_trace()` -> `load_project_instructions_from_cwd()` -> `native_loop_input()` -> `AgentLoop.run()` -> `OpenAiProvider.complete()` -> `ToolBroker.execute()` -> `AgentLoopResult.to_run_status()` -> `AppServer.append_native_trace()`。`AppServer.turn_start()` 先读取 thread 并校验 Windows restricted-token command sandbox capability；capability 不满足时直接返回 JSON-RPC error，不生成 turn、trace 或 provider request。校验通过后，`SessionStore.create_turn_with_input_and_trace()` 生成对象并写入 sqlite store：turn、用户输入 item 和 app-server trace。随后 `load_project_instructions_from_cwd()` 先 canonicalize thread `cwd`，以最近 `.git` marker 确定 workspace root，只遍历 root 到实际 `cwd` 的目录链；每个候选 `AGENTS.md` canonicalize 后必须仍位于 workspace 内，symlink/junction 越界、非普通文件、无效 UTF-8、读取失败、单文件超过 32 KiB 或总量超过 64 KiB 都返回结构化 `ProjectInstructionError` 并 fail closed，完全缺失则正常继续。`native_loop_input()` 把合并文本写入仅内存使用的 `AgentLoopInput.project_instructions`，`AgentLoop.run()` 先生成 developer message，再追加原有 user/history/tool message 并构造 `ModelTurnRequest`；项目指令不会拼进 user text。通过 `OpenAiProvider` 调用真实 provider，解析 assistant message / tool calls，再由 `ToolBroker`、`PolicyEngine` 和 `WorkspaceTools` 执行被允许的 workspace tool 或 command tool。工具结果生成 `ToolResult` 并回送给后续模型 turn；需要人工或 policy approval 时生成 `ApprovalRequest` 与 `PendingToolCall`，由 app-server 持久化到 approval / pending_tool_calls 表。完成时 `AgentLoopResult.to_run_status()` 生成 `AgentRunStatus`，app-server 写入 `agent_loop` trace 摘要并更新 turn 状态；最终答案只在 `completed` 且非空时写为 agent message item，失败、blocked、cancelled 不伪造 assistant delta。
...
`AgentLoopInput` 由 app-server 从 Rust thread/turn/user input 和已验证的 workspace `AGENTS.md` 生成。`project_instructions` 使用 serde/schema skip，只能由 Rust loader 在进程内设置；`AgentLoop` 把它投影为第一条 `developer` message，不写入 user item、input serialization、tool schema、history store 或 trace metadata。**边界**：只在 native loop 内存中使用；不把原始 provider request、raw prompt、指令源绝对路径或 secret 写入 trace。
...
- `load_project_instructions_from_cwd()` 生成按 root -> cwd 排序的 `ProjectInstructions` 或结构化 `ProjectInstructionError`；`native_loop_input()` 只把验证后的合并文本传给 `AgentLoopInput.with_project_instructions()`。`AgentLoopInput.new()` 设置 thread/turn/run/session/task identity、模型偏好、最大 turn 数和初始用户输入。
...
- `AgentLoop.run()` 消费 `AgentLoopInput`、`OpenAiProvider`、`ToolBroker`、`PolicyEngine` 和 `WorkspaceTools`；有项目指令时，model request 的 message 顺序从 developer 开始，user 内容保持原样。
...
- `AgentLoopInput` 和 `AgentLoopResult` 本体不作为独立文件落盘；`project_instructions` 字段禁止 serde/schema 投影，`ProjectInstructions.sources` 也不进入 store。
...
- app-server 写 `component="agent_loop"` 的 trace summary，只包含 status、completed、run_id、session_id、task_id、model_turns、tool_calls、approval_count、audit_events 和已脱敏 error；不记录 `AGENTS.md` 内容、源路径或 hidden env。
...
- workspace/cwd canonicalize 失败、cwd 越界、`AGENTS.md` symlink/junction 越界、metadata/read/UTF-8 错误、非普通文件或字节上限超限都返回结构化 project-instruction error；普通 turn 转为 failed status，approval resume/eval 同样 fail closed，且不会调用 provider。
