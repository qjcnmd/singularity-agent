...
- Python runtime module smoke失败：doctor仍保持schema v2和原有`available`计算，但在`diagnostics`写`python_runtime_environment_blocker`，包含`failure_type`、失败账户角色、network mode、模块状态、OpenSSL config/provider/cert/TEMP访问信息、runtime target hash、runner exit/stdout/stderr摘要、restricted/low-integrity/private desktop/Job Object evidence及hash/redacted路径；`_ssl.pyd`加载、`libssl/libcrypto`依赖、OpenSSL provider/config、证书路径、TEMP/profile、DLL search path和low-integrity初始化失败会提前细分，不能等到任务中后段才模糊归类为unknown。
