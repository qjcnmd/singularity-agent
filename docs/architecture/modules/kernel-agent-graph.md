...
组件构图失败抛 `AgentGraphInitializationError`，bootstrap 包装为 `KernelBootstrapError`，同时把 run 标记为 failed 并保留可复制 session 命令；task 运行异常/取消由 `KernelError`/`CancellationError` 与 `FAILED/BLOCKED/CANCELLED` lifecycle 表达，diagnostics 保留异常类型/脱敏消息并执行 shutdown。恢复门禁发现 external change、rollback conflict、unfinished mutation、stale lock、leftover sandbox 或 pending/running tool 时，`AgentKernel.run_task()` 在模型调用前返回 blocked/needs-review 报告。
