...
在 `workspace-write` 且 `PolicyDecision.outcome=sandbox_required` 时，普通本地验证命令不得走 `local_process`。Windows elevated backend 可用时，`CommandResult.backend` 为 `windows_elevated`，`sandbox_enforcement="strict"`，`filesystem_isolation="native_os_sandbox"`。elevated 因本机环境 blocker 不可用时，`CommandResult.backend` 可为 `windows_unelevated`，`sandbox_enforcement="reduced"`，`enforcement_status="degraded"`，`filesystem_isolation="workspace_policy_enforced"`，并在 sandbox report/metadata 中记录 `fallback_used`、`fallback_reason`、`elevated_available=false` 和脱敏 `elevated_blocker_summary`。`windows_unelevated` 不会被标为 native/elevated OS sandbox。elevated 与 unelevated 都不可用时，结果是 `ExecutionStatus.BACKEND_ERROR` / `error_code=sandbox_unavailable`，不是普通本地执行。

只有会话级 `PermissionProfile.profile == "danger-full-access"` 且调用方仍显式要求 sandbox 时，`SandboxManager` 才允许在 native/reduced backend 不可用或无法满足能力要求时走 relaxed `local_process`。该分支不改变 policy/protected path 前置检查，`CommandResult.backend` 为 `local_process`，`isolation_report["filesystem_isolation"]` 保持 `workspace_cwd_advisory`，`isolation_report["sandbox"]` 与 metadata 必须写入 `sandbox_mode="danger-full-access"`、`sandbox_backend="local_process"`、`sandbox_enforcement="relaxed"`、`fallback_used=true`、`used_local_process_fallback=true` 和 `local_process_fallback_reason`。`read-only` 与 `workspace-write` 不使用 local process 分支。
...
`PolicyEngine` 返回 `REQUIRE_REVIEW`/`DENY`、cwd denied、sandbox setup/backend error、timeout/idle timeout、kill 或非零退出时生成非成功 `CommandResult`；process API 通过 `status`、`error_code`、`killed_reason` 表达失败，不把启动失败登记为 running session。`workspace-write` 下 elevated backend unavailable 时可降级 `windows_unelevated`，但 `backend` 不得是 `local_process`；如果两个 Windows backend 都不可用，`sandbox_availability` 必须说明 backend 状态，输出仍按 `SecretRedactor` 和 output limit 处理。`danger-full-access` 的 relaxed fallback 是显式会话模式，不得被当作 native sandbox enforcement，也不得让 `filesystem_isolation` 显示为 `native_os_sandbox`。
