...
- `assess_verification_contract_satisfaction` fail-closed: when an active repair contract exists but `VerificationContract.steps` is empty → `satisfied=False` (fail-closed); when the planner is in `repairing_failures`, `running_verification`, or `finalizing` and repair-plan evidence exists without an authoritative `repair_contract` → `satisfied=False` with `reason="repair_contract_missing"`; when no active repair exists and steps is empty → `satisfied=True` (no verification needed). When steps exist but `verification_results` is empty → `satisfied=False`. When steps exist and verification ran but `step_evidence` is missing (command didn't match contract step) → `satisfied=False`.
