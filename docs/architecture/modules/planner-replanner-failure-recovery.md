...
- `Planner._active_tool_scope()` 生成 `_ToolScope`，`Planner.authorize_tool_call()` 生成 `AuthorizationDecision`；其中 deny 分支通过 `_deny_tool_call()` 统一记录 planner event 并返回 decision。`Replanner.decide()` 生成规则层面的 `ReplanDecision`，`Planner.replan()` 应用该 decision 并处理持久化/事件/预算状态，`RiskEscalator.evaluate_action()` 生成 `RiskEscalation`。`Planner.finalize()` 委托 `Finalizer.build()` 生成 `FinalReport`。
