...
Singularity routes process execution through a Command Execution. A shell is not a normal tool: it can execute project code, mutate the workspace, leak environment secrets, access the network, start background processes, and leave child processes behind. The component makes those effects explicit before a command is started and keeps the result auditable afterward.
...
ToolExecutor
  ...
  -> CommandPlan
  -> CommandPolicy
  -> PolicyEngine sandbox check
  -> EnvPolicy + ResourceLimits + OutputCollector
  -> ExecutionBackend or SandboxManager
  -> ProcessSupervisor
  -> workspace side-effect tracking
  -> trace + context observation
```

Tool handlers are not allowed to spawn processes directly. `ToolExecutor` rejects any `SHELL` tool unless its `ToolSpec` declares `uses_command_executor=true`.
...
`ToolExecutor` owns tool registration, schema validation, tool policy, handler timeout, and compact `ToolResult` wrapping. It only lets command tools enter the command layer when `uses_command_executor=true`.

`CommandExecutor` owns command planning, command policy, env policy, network/filesystem declarations, process execution, resource limits, output collection, side-effect tracking, command observations, and command audit events.

`WorkspaceMutationManager` owns model-authored file edits. Command-generated changes are tracked as command side effects and are not mixed with model apply operations or mutation journals.

`VerificationRunner` decides what checks to run, while CommandExecutor runs the approved command and returns `CommandResult` with semantic statuses such as `tests_failed`, `build_failed`, `lint_failed`, and `typecheck_failed`. Direct `run_command` tool calls reject verification-like commands with `verification_runner_required`; this keeps tests, lint, typecheck, builds, and syntax checks on the verification planning path instead of ad-hoc shell execution.

When `PolicyEngine` returns `sandbox_required`, CommandExecutor calls `SandboxManager` and does not spawn the command through `LocalProcessBackend`.

`GitClient` is implemented as a local-only status/diff/commit wrapper. It invokes the configured git executable directly, never executes user-provided shell command strings, never pushes or pulls, and stages only explicit paths scoped to the configured workspace root.
...
`LocalStagingBackend` is implemented through `SandboxManager`. It provides practical local copy-on-write isolation, not a hard OS security boundary. It reports `network_isolation=false`, `memory_limit=false`, and `process_limit=false`, and fails closed when policy requires those unsupported capabilities.
...
The command executor captures a workspace snapshot before and after execution. It skips VCS internals, execution artifacts, caches, virtual environments, `node_modules`, and bytecode directories.
...
This separation matters because a formatter, package manager, test, or code generator can alter files without passing through `WorkspaceMutationManager`.
...
`GitClient` owns local Git status, diff statistics, scoped staging, and local commits. CommandExecutor trace git summaries are only command side-effect context, not a replacement for GitClient.
...
Command error codes are part of the execution contract:
...
ToolExecutor command tool registration path
```
