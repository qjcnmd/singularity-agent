...
Singularity v0.0.15 adds a dedicated instruction and prompt compilation layer. The goal is not a larger system prompt string; it is a execution boundary that turns typed instruction sources into a provider-ready `PromptBundle` and a trace-safe `PromptManifest`.

## Boundary

`PromptAssemblyPipeline` owns instruction source collection, priority, trust, conflict resolution, prompt injection detection, prompt compilation, manifest creation, and trace events.

It does not execute tools, call models, approve policy decisions, mutate files, run commands, inspect Git state, or resolve user approvals. Those remain owned by ToolExecutor, ModelRunner, PolicyEngine, ApprovalGate, WorkspaceMutationManager, CommandExecutor, VerificationRunner, and SandboxManager.
...
AgentLoop
  ...
  -> PromptAssemblyPipeline builds PromptBundle and PromptManifest
  -> ModelRunner receives PromptBundle.messages
...
- `priority`: system invariant, Singularity developer, user session, user task, project instruction, component observation, retrieved content, or model generated.
- `trust_level`: trusted system, trusted Singularity, trusted user, project declared, component observation, untrusted content, or model generated.
- `scope`: component, purpose, path, tool, session-only, and task-only filters.
...
Lower-priority content cannot override higher-priority instructions. Project instructions cannot override user task/session, Singularity, or system rules. Tool output, command output, project files, README content, model output, and summaries are data unless a higher-trust component classifies them otherwise.
...
By default warnings are recorded and isolated, not automatically denied. `fail_on_critical_injection=True` can turn critical warnings into a structured component failure.
...
- developer: Singularity behavior, tool protocol, evidence/reporting rules, trusted component summaries
- user: current user task/session instruction
- context data: component observations, project instruction data, retrieved content, tool output, command output, summaries
...
Trace events added by this component:
...
This gives the user and future component layers prompt provenance without leaking full prompts.
