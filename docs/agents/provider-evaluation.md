...
- 用户级 Provider 配置持久化在默认 Singularity home：`config.json` 与唯一的 `auth.json`。真实调用需要认证时，先核对这两个文件和脱敏后的配置诊断；凭据只来自 `auth.json`，不得读取、打印或手工转录密钥。
- 真实调用使用临时目录、临时 SINGULARITY_HOME 和配置副本，并保存可复核证据。核心链路优先通过 `C:\Users\Lenovo\Desktop\Singularity-Evaluator` 的黑盒 evaluator 运行；本仓库只保留崩溃恢复与协议词形两类护栏测试（凭据脱敏与有界读取由实现与文档合同保证，不设专项测试），行为回归以评估器为准。只有配置/认证诊断明确证明缺失或无效，或者真实请求返回可定位的外部阻断时，才能把该项报告为未执行或阻断。
