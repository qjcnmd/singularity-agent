...
- 用户级 Provider 配置持久化在默认 Singularity home：`config.json` 与唯一的 `auth.json`。真实调用需要认证时，先核对这两个文件和脱敏后的配置诊断；不得因当前进程没有 API key 环境变量而判定凭据不存在，也不得读取、打印或手工转录密钥。
- 真实调用使用临时目录、临时 SINGULARITY_HOME 和配置副本，并保存可复核证据。核心链路优先通过 `C:\Users\Lenovo\Desktop\Singularity-Evaluator` 的黑盒 evaluator 运行；本仓库只保留 `entry_contract`、`ui_pty` 和 app-server 进程级边界测试。只有配置/认证诊断明确证明缺失或无效，或者真实请求返回可定位的外部阻断时，才能把该项报告为未执行或阻断。
