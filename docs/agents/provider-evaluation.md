...
- 真实调用使用临时目录、临时 SINGULARITY_HOME 和配置副本，并保存可复核证据。核心链路优先通过 `C:\Users\Lenovo\Desktop\Singularity-Evaluator` 的黑盒 evaluator 运行；本仓库只保留崩溃恢复、安全与协议词形三类护栏测试，行为回归以评估器为准。只有配置/认证诊断明确证明缺失或无效，或者真实请求返回可定位的外部阻断时，才能把该项报告为未执行或阻断。
