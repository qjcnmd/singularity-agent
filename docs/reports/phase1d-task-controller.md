# Phase 1D Resolution Report: RunController / Lifecycle Reducer
...
- `AgentLoop.run()` directly owned the outer turn loop.
- `ExecutionOutcome` existed, but lifecycle reduction happened inside the agent and planner.
- `Planner` directly mutated planner status in several methods.
- Tool Protocol recovery produced string `next_action` values, but no task-level event model consumed them.
- Planner checkpoint/resume existed through `PlannerStore`, but no `RunController` adapter preserved lifecycle waiting states.
...
3. Move the outer `AgentLoop.run()` loop into `RunController.run_loop()`.
4. Route model/protocol/completion outcomes through `RunController.apply_outcome()`.
5. Map Tool Protocol `next_action` and recovery results into `RunControlEvent`.
...
- `src/singularity/run_controller.py`
  - Added `RunLifecycleStatus`, `RunControlEventKind`, `RunControlEvent`, `RunCheckpointStore`, `RunOutcomeReducer`, and `RunController`.
- `src/singularity/agent.py`
  - Delegates the outer turn loop to `RunController.run_loop()`.
  - Routes outcomes through `RunController.apply_outcome()`.
  - Routes Tool Protocol `next_action` through `RunController.apply_protocol_result()`.
...
- `tests/agent_component_helpers.py`
...
```powershell
.\.venv\Scripts\python.exe -m pytest tests\test_task_controller.py tests\test_agent_task_outcome.py tests\test_agent.py tests\test_planner.py tests\test_tool_protocol_recovery.py tests\test_tool_protocol_state.py tests\test_verification_runner.py tests\test_context_recovery_production.py --basetemp work/pytest-tmp
```
...
- Successful low-level tool observations are still recorded by their existing components; the controller consumes protocol turn results and task outcomes for lifecycle decisions.
