...
Phase 1I aligns documentation, runtime naming, config precedence, effective config evidence, and sandbox capability claims with the implemented Python CLI runtime.
...
- `SandboxRuntime` fail-closed behavior for requests that require unavailable hard isolation.
...
- README and runtime map did not mark capabilities as `implemented`, `partial`, or `planned`.
- Runtime docs used `ContextRuntime` as the runtime name even though the implemented runtime class is `ContextManager`; `ContextRuntime` is a context-source enum.
...
1. Keep the implementation inside the existing Python CLI runtime and documentation surfaces.
...
- `src/singularity/kernel/bootstrap.py`
  - Emits an effective config trace event with runtime `config`.
  - Uses source-aware config summaries in bootstrap failure reports.
- `src/singularity/kernel/runtime.py`
  - Uses source-aware config summaries in normal kernel final reports.
- `src/singularity/sandbox/runtime.py`
  - Adds `SandboxRuntime.capability_summary()`.
...
- `src/singularity/kernel/graph.py`
  - Records sandbox capability during runtime graph wiring.
- `README.md`
  - Adds runtime capability status and honest config/sandbox claims.
- `docs/architecture/runtime-map.md`
  - Replaces `ContextRuntime` runtime naming with `ContextManager` and adds capability status.
...
- `docs/architecture/sandbox-isolation-runtime.md`
  - Documents Docker hard isolation versus local staging.
- Tests
  - Adds coverage in `tests/test_production_baseline_alignment.py`, `tests/test_cli.py`, `tests/test_kernel_bootstrap.py`, `tests/test_runtime_graph.py`, `tests/test_docs_consistency.py`, and `tests/test_sandbox_runtime.py`.
...
```powershell
.\.venv\Scripts\python.exe -m pytest tests\test_production_baseline_alignment.py tests\test_cli.py tests\test_runtime_graph.py tests\test_kernel_bootstrap.py tests\test_docs_consistency.py tests\test_sandbox_runtime.py --basetemp work\pytest-tmp-phase1i-focused-after-source
```
...
- `.singularity/config.toml` is intentionally limited to non-secret runtime settings. `SINGULARITY_API_KEY` remains environment-only.
...
- The effective config trace event uses the existing `context.observation_added` event type with runtime `config` rather than adding a new trace enum.
