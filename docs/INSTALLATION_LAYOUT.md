...
The Installation Layout makes Singularity usable as an installed local CLI instead of a source-directory-only tool.

It owns package metadata, user user data directories, initialization, version output, health checks, repair, export, and safe uninstall. Git commands live in `GitClient`, not Installation Layout. Installation Layout has no branch, commit, PR, or push commands.
...
`UserDataPaths` resolves the user data root from, in order:
...
`doctor` is read-only. It reports Python compatibility, source-vs-installed execution, directory access, config schema, critical component config, optional dependencies, and pending migrations.

`repair` creates missing user data directories and missing default files. It does not overwrite an existing config.
...
`export` writes a zip with relative paths and a manifest, avoiding embedded absolute component-root paths.
