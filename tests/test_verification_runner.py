...
def test_verification_plan_separates_required_optional_skipped_and_blocked(tmp_path: Path) -> None:
    ...
    docs_plan = component.plan_verification(
        changed_files=["README.md"],
        task_intent="docs",
    )
    source_plan = component.plan_verification(
...
def test_verification_runner_executes_checks_through_command_executor_and_records_trace(tmp_path: Path) -> None:
    ...
    plan = component.plan_verification(changed_files=["src/app.js"], task_intent="code")
...
def test_explicit_smoke_command_records_exit_stdout_and_stderr(tmp_path: Path) -> None:
    ...
    smoke_check = next(
        check for check in plan.required_checks if check.kind == CheckKind.VERIFICATION_SMOKE
    )
    ...
    smoke = next(
        ...
        if result["kind"] == CheckKind.VERIFICATION_SMOKE.value
    )
...
def test_plan_verification_uses_contract_smoke_commands_when_missing(tmp_path: Path) -> None:
    ...
    plan = component.plan_verification(changed_files=[], task_intent="verify contract")
    observation = component.run_plan(plan.id)
    smoke = next(
        result
        for result in observation["verification"]["results"]
    ...
    smoke_check = next(check for check in plan.required_checks if check.kind == CheckKind.VERIFICATION_SMOKE)
    assert smoke_check.command is not None
    assert smoke_check.command.argv == ["python", "quicksort.py"]
    assert fake.calls[0].argv == ["python", "quicksort.py"]
    assert smoke["evidence"]["exit_code"] == 0
...
def test_post_verification_review_report_is_written_to_observation(tmp_path: Path) -> None:
    ...
    (tmp_path / "package.json").write_text(json.dumps({"scripts": {"test": "pytest"}}), encoding="utf-8")
    review = ReviewPipeline(tmp_path, enable_model_critic=False)
    component = VerificationRunner(tmp_path, command_executor=fake, review_pipeline=review)
...
def test_verification_runner_sends_structured_observation_to_memory(tmp_path: Path) -> None:
    request = CommandRequest(argv=["pytest"])
    fake = FakeCommandExecutor(
        [
            command_result(
    ...
    (tmp_path / "package.json").write_text(json.dumps({"scripts": {"test": "pytest"}}), encoding="utf-8")
    component = VerificationRunner(tmp_path, command_executor=fake)
    component.memory_pipeline = memory
...
def test_verification_tool_observation_enters_context_manager(tmp_path: Path) -> None:
    ...
    register_verification_tools(registry, VerificationRunner(tmp_path))
