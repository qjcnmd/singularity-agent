...
def _runtime(tmp_path: Path, *, index=None, verification=None, review=None) -> EditRuntime:
    mutation = MutationRuntime(tmp_path, project_index_runtime=index)
    return EditRuntime(
        tmp_path,
        mutation_runtime=mutation,
        project_index_runtime=index,
        verification_runtime=verification,
...
def test_structured_json_update_and_python_symbol_replace(tmp_path: Path) -> None:
    ...
    json_result = runtime.apply_intent(
        EditIntent(
            summary="update json",
            operations=[
                EditOperation(
    ...
    py_result = runtime.apply_intent(
        EditIntent(
...
def test_validation_blocks_forbidden_path_hash_stale_context_conflict_and_high_impact(tmp_path: Path) -> None:
    ...
    forbidden = _runtime(tmp_path).apply_intent(
        EditIntent(
            summary="secret",
            operations=[EditOperation(kind="create_file", path=".env", content="TOKEN=x\n")],
        )
    )
    stale = _runtime(tmp_path).preview_intent(
        EditIntent(
            summary="stale",
    ...
    conflict = _runtime(tmp_path).preview_intent(
        EditIntent(
            summary="ambiguous",
            operations=[EditOperation(kind="replace_text", path="dup.txt", old_text="same", new_text="other")],
        )
    )
    high_impact = _runtime(tmp_path, index=_Index(high=True)).preview_intent(
        EditIntent(
            summary="impact",
...
def test_repair_refreshes_stale_hash_and_context_fallback_succeeds(tmp_path: Path) -> None:
    ...
    refreshed = runtime.apply_intent(
        EditIntent(
...
def test_apply_records_post_apply_impact_and_verification_plan(tmp_path: Path) -> None:
    ...
    runtime = _runtime(tmp_path, index=_Index(), verification=verification)
...
def test_pre_edit_review_blocks_apply_before_preview_or_mutation(tmp_path: Path) -> None:
    ...
    runtime = _runtime(tmp_path, review=review)
