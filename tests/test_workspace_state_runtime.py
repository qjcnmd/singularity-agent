...
def test_file_snapshot_records_hash_metadata_encoding_line_endings_symlink_and_class(
    ...
    runtime = LocalWorkspaceStateRuntime(tmp_path)
    runtime.begin_session(task_id="task_1")
    snapshot = runtime.snapshot_file("README.md")
    ...
    if link is not None:
        link_snapshot = runtime.snapshot_file("readme-link.md")
        assert link_snapshot.is_symlink is True
        assert link_snapshot.symlink_target


def test_mutation_runtime_records_agent_owned_changes_in_state_journal_and_trace(
    tmp_path: Path,
) -> None:
    ...
    mutation = MutationRuntime(tmp_path, trace=trace, state_runtime=state)
...
def test_external_change_detection_and_snapshot_mismatch_blocks_mutation(tmp_path: Path) -> None:
    ...
    mutation = MutationRuntime(tmp_path, state_runtime=state)
    result = mutation.apply_operations(
        [
            ReplaceText(
                path="app.py",
...
def test_artifact_store_journal_recovery_health_and_context_observation(tmp_path: Path) -> None:
    ...
    restarted = LocalWorkspaceStateRuntime(tmp_path)
