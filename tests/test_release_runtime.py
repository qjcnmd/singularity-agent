...
def test_runtime_paths_honor_miniharness_home(monkeypatch, tmp_path: Path) -> None:
    ...
    assert paths.mode == RuntimeMode.USER
...
def test_doctor_json_reports_runtime_health_without_real_home(
    monkeypatch,
    tmp_path: Path,
) -> None:
    monkeypatch.setenv("MINIHARNESS_HOME", str(tmp_path / "runtime"))
...
def test_migration_failure_rolls_back_config_and_manifest(tmp_path: Path) -> None:
    ...
    original_config = {"schema_version": 1, "runtime": {"mode": "user"}, "policy": {}, "sandbox": {}, "model": {}, "provider": {}}
    ...
    def boom(runtime_paths):
        atomic_write_json(runtime_paths.config_file, {"schema_version": 1, "broken": True})
...
def test_uninstall_blocks_unowned_runtime_home(monkeypatch, tmp_path: Path) -> None:
