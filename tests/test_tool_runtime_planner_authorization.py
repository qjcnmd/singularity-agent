...
def test_planner_denial_prevents_handler_execution(tmp_path: Path) -> None:
    calls: list[str] = []
    runtime = make_runtime(tmp_path, planner=DenyingPlanner(allowed=False), handler_calls=calls)
...
def test_planner_absent_standalone_mode_allows_read_tools(tmp_path: Path) -> None:
    ...
    result = runtime.execute_tool_call(make_tool_call("read_file", {"path": "README.md"}))
...
def test_planner_denial_records_policy_observation_when_available(tmp_path: Path) -> None:
    ...
    runtime = make_runtime(tmp_path, planner=planner, handler_calls=calls)
