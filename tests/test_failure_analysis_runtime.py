...
from singularity.verification import FailureAnalysisRuntime, FailureType, VerificationRuntime
from tests.test_verification_runtime import FakeCommandRuntime, command_result
...
def test_same_failure_repeated_routes_to_no_progress() -> None:
    runtime = FailureAnalysisRuntime(max_same_failure_retries=1)
    result = {
    ...
    first = runtime.analyze_result(result, changed_files=["app.py"])
    second = runtime.analyze_result(result, changed_files=["app.py"])
...
def test_golden_failure_repair_rerun_passes(tmp_path: Path) -> None:
    ...
    failed = runtime.run_plan(plan.id)
    (tmp_path / "app.py").write_text("FIXED = True\n", encoding="utf-8")
    passed = runtime.rerun_check(
        plan_id=plan.id,
