...
5. 验证：按最终 diff 的实际风险选择最小充分检查；文档、提示词和纯格式修改不得机械运行代码测试、真实 Provider smoke、Evaluation 或等待 CI；代码、协议、持久化、并发、安全和发布入口变更才按影响范围增加定向测试、构建、真实链路或 Evaluation。
6. 交付：push 前只执行当前 diff 必需的本地检查；push 后是否等待 CI 取决于变更风险、分支保护和仓库工作流，不得把自动触发的 CI 当成纯文档提交的默认阻断条件。
...
CLI、TUI 和即将接入的 Desktop 应消费同一核心 Agent 能力。
...
按最终 diff 的实际风险选择最小充分验证；验证对象是本次变更可能破坏的合同，不是“所有检查都跑一遍”。

变更类型与最低验证要求：

- 纯文档、决策记录、注释或提示词：检查最终内容、归属和链接，运行 `git diff --check`；只有涉及文档站、生成文件、导航、翻译或可见行为时，才运行对应的窄检查。默认不运行 Rust 测试、真实 Provider smoke、Evaluation 或等待 CI。
- 纯格式、重命名或机械整理：检查 diff、引用和生成状态；无行为变化时不运行代码测试。
- 代码内部逻辑：运行受影响 crate 的定向测试、check 或 clippy。
- 公共协议、持久化、并发、安全、Provider 或客户端链路：在定向测试之外，增加相应集成测试或细粒度真实链路验证；只有确实改变完整任务行为时才运行 Evaluation。
- CI 配置、构建脚本、发布入口或分支保护相关文件：按其实际影响验证，并在 push 后关注对应 CI。
...
大型改动可以按独立、可审查、可回滚的阶段创建本地提交；简单修改不为了形式完整强制拆 commit。push 只对最终候选执行：先完成与 diff 风险相称的独立审查和本地检查，再 push；只有代码行为、协议、构建、发布或分支保护要求时，才把 CI 结果作为交付门禁。纯文档提交即使触发 CI，也默认只记录其状态，不因等待 CI 扩大本地验证范围。一次 push 授权不代表可以提前 push 或跳过这些适用门禁。
