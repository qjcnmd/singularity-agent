...
- 可由真实模型产品调用覆盖的行为不保留 mock、fake 或模拟测试副本；真实调用建立等价或更强证据后，删除对应测试及其专用 fixture、helper 和分支。只有外部模型无法可靠构造的畸形 wire、精确错误码、事务故障、权限拒绝、取消竞态、崩溃恢复和 no-replay 等确定性边界才保留 test double，并在测试语义中明确其不可替代的不变量。
...
- 测试集合按行为证明关系保持最小：若测试 B 的可观察行为与断言完整证明测试 A、C、D 的全部不变量，删除 A、C、D 及其专用 fixture/helper；只有在同一环境的可比实测中 B 的耗时大于 A+C+D 总和时才保留较快的拆分测试。覆盖关系和耗时必须来自当前源码与实际运行，不从测试名称或历史结果推断。
